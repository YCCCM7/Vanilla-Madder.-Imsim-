	...
	function BeginState()
	{
		...
		if (player != None)
		{
			...
			if (VMP != None)
			{
				...
				SBA = SodaBuffAura(VMP.FindInventoryType(class'SodaBuffAura'));
				if (SBA == None)
				{
					SBA = Spawn(class'SodaBuffAura');
					SBA.Frob(VMP, None);
					SBA.Activate();
				}
				else
				{
					SBA.Charge = SBA.Default.Charge;
				}
			}
		}
		
		if ((VMP != None) && (VMP.bAssignedFemale)) Player.PlaySound(sound'VMDFJCBurp');
		else Player.PlaySound(sound'MaleBurp');
		UseOnce();
	}
Begin:
}
