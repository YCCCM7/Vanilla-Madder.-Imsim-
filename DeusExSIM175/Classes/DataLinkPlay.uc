...
var localized S_InfoLinkNames infoLinkNames[64];
...
//MADDERS, 4/28/21: New definition for AbortAndSaveHistory, courtesy of DXT.
function AbortAndSaveHistory()
{
    ...
    {
        PlayNextEvent();
        // Go through all of the current infolink and fire everything correctly
        while (currentEvent != None)
        {
            GotoState('PlayEvent');
            SetupEvent();
        }
        AbortDataLink(); // Close it
        while (FireNextDataLink()) // Move to the next infolink (if there is one), repeat
        {
            while (currentEvent != None)
            {
                GotoState('PlayEvent');
...
function EEventAction SetupEventSpeech( ConEventSpeech event, out String nextLabel )
{
	...
	//MADDERS, 4/14/21: Datalink merge, bby. Give us more time.
	if (VMP != None)
	{
		switch(VMP.BindName)
		{
...
function String GetDisplayName(String bindName)
{
	...
	if (DisplayName ~= "")
	{
		return AttemptSpacify(BindName);
	}
	
	return displayName;
}

//MADDERS, 4/18/21: HAAACKS... AAAAAH!
function string AttemptSpacify(string Conv)
{
