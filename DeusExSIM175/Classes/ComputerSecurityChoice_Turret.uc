...
//MADDERS, 1/14/21: Detect hack status with this'n.
...
{
	Super.ButtonActivatedRight(buttonPressed);
	SetTurretState();
...
function SetTurretState()
{
	...
	//MADDERS: Without the proper augment, we can't turn turrets on our enemies.
	VMP = VMDBufferPlayer(GetPlayerPawn());
	
	if ((VMP != None) && (WinTerm.bHacked) && (!VMP.HasSkillAugment("ComputerTurrets")))
	{
      		if (GetValue() > 1)
		{
         		SetValue(0);
