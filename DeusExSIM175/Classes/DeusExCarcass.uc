...
function bool ShouldExplode()
{
	...
	local int LastAmmoRand;
 	
	if (bExplosive) return true;
...
function DumpItemInventory()
{
	...
	local VMDBufferPlayer VMP;
 	...
 	{
  		Explode();
  		if (bExplosive) return; //Only return when we were already gonna 'splode!
 	}
...
function InitFor(Actor Other)
{
	...
	if (Other != None)
	{
		...
		if (VMBP != None)
		{
			...
			for (i=0; i<8; i++)
			{
				if (VMBP.OverhaulTex[i] != None)
				{
					Multiskins[i] = VMBP.OverhaulTex[i];
				}
			}
	...
	for(TItem = Inventory; TItem != None; TItem = NextItem)
	{
		...
		else if (DXP != None)
		{
			...
			if (TCharge != None)
			{
				...
				if (TCharge.Charge <= TCharge.Default.Charge * 0.02)
				{
					TCharge.Destroy();
					continue;
				}
		...
		if (DXW != None)
		{
			...
			if ((FindWeapon != None) || ((FindWeapon == None) && (!FindAdvancedInventorySlot(Player, DXW, true))))
			{
				if (DXW.PickupAmmoCount > 0 || DXW.VMDIsMeleeWeapon())
				{
					...
					if ((FindAmmo != None) && (FindAmmo.AmmoAmount < FindAmmo.VMDConfigureMaxAmmo()) && (!DXW.VMDHasJankyAmmo() || FindWeapon != None))
					{
						...
						if (DXW.PickupAmmoCount > 0)
						{
							...
							{
								Player.ClientMessage(DXW.PickupMessage @ DXW.ItemArticle @ DXW.ItemName @ "("$DXW.PickupAmmoCount$")", 'Pickup');
							}
							else
							{
								Player.ClientMessage(FindAmmo.PickupMessage @ FindAmmo.ItemArticle @ FindAmmo.ItemName @ "("$DXW.PickupAmmoCount$")", 'Pickup');
							}
						}
					...
					else
					{
						...
						else
						{
							...
							Player.ClientMessage(SprintF(MsgCannotPickup, DXW.ItemName));
						}
					}
				}
				...
				if ((FindWeapon == None) && (!FindAdvancedInventorySlot(Player, DXW, true)))
				{
					...
					{
						VMP.ClientMessage(Sprintf(VMP.ItemRefusedString, DXW.ItemName));
					...
					else
					{
						...
						{
							VMP.ClientMessage(Sprintf(VMP.InventoryFullNull, DXW.ItemName));
						}
						else
						{
							if (VMP != None)
							{
								VMP.ClientMessage(SprintF(VMP.InventoryFullFeedback, DXW.ItemName, VMP.VMDConfigureInvSlotsX(DXW), VMP.VMDConfigureInvSlotsY(DXW)));
							}
							else if (DXW != None)
							{
								VMP.ClientMessage(SprintF(Player.InventoryFull, DXW.ItemName));
							}
				...
				if ((FindWeapon != None) && (!bWeaponWasDropped))
				{
			...
			if (!bPickedItemUp)
			{
				...
				if ((Player.Inventory == None || Level.Game.PickupQuery(Player, DXW)))
				{
					...
					if (Player.HandleItemPickup(DXW))
					{
						...
						Player.ClientMessage(DXW.PickupMessage @ DXW.ItemArticle @ DXW.ItemName, 'Pickup');
...
function bool VMDRemoveInventory(Inventory TInv, bool bDrop)
{
	...
	else
	{
		...
		{
			bDropWon = DeusExAmmo(TInv).VMDDropFrom(TLoc, true);
		}
		if (DeusExPickup(TInv) != None)
		{
			bDropWon = DeusExPickup(TInv).VMDDropFrom(TLoc, true);
		}
		if (DeusExWeapon(TInv) != None)
		{
			TAmmoCount = Max(0, DeusExWeapon(TInv).PickupAmmoCount);
			bDropWon = DeusExWeapon(TInv).VMDDropFrom(TLoc, true);
		}
		else
		{
		...
		if (bDropWon)
		{
			...
			if (DeusExWeapon(TInv) != None)
			{
				DeusExWeapon(TInv).PickupAmmoCount = TAmmoCount;
			}
		}
	}
