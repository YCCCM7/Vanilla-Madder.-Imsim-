...
function UpdateInfo()
{
	...
	local int QuanReqA, QuanReqB, QuanReqC, ItemOwnedA, ItemOwnedB, ItemOwnedC, TarItemOwned,
			ScrapCost, ScrapGain, QuanMult, CraftQuan, BreakdownQuan, ComplexityNeeded, SkillNeeded, SkillLevel;
	...
	else if ((VMP != None) && (VMP.SkillSystem != None))
	{
		if ((TarTile != None) && (TarTile.CurItem != None))
		{
			...
			if (bHasBreakdown)
			{
				...
				ScrapGain = CF.GetMechanicalBreakdownPrice(TarItem) * BreakdownGainTweak;
				
				BreakdownQuan = CF.GetMechanicalBreakdownQuanNeeded(TarItem);
				
				RequirementLabels[5].SetText(SprintF(MsgBreakdownGain, ScrapGain*QuanMult));
			...
			if (bHasBreakdown)
			{
				if (QuanMult > 1)
				{
					BuildStr[2] = SprintF(MsgBreakdownParamsA, BreakdownQuan, ScrapGain)$SprintF(MsgBreakdownParamsB, QuanMult, BreakdownQuan*QuanMult, ScrapGain*QuanMult);
				}
				else
				{
					BuildStr[2] = SprintF(MsgBreakdownParamsA, BreakdownQuan, ScrapGain);
				}
...
{
	local bool bHasTalent;
	local int SkillLevel, SkillNeeded, QuanMult, ScrapGain, BreakdownQuan, AmountNotAdded;
	...
	if ((bCanBreakdown) && (CF != None) && (SelectedTile != None) && (SelectedTile.CurItem != None) && (VMP.SkillSystem != None))
	{
		...
		if (VMP.bUseInstantCrafting || bHasRepairBot)
		{
			...
			AmountNotAdded = (ScrapGain*QuanMult) - (VMP.MaxScrap - VMP.CurScrap);
			if (AmountNotAdded > 0)
			{
			...
			VMP.AddScrap(ScrapGain*QuanMult, false);
			RefreshItemTiles();
		}
		else
		{
			VMP.VMDCraftingStartBreakdown(TarItem, BreakdownQuan, ScrapGain, false, false, QuanMult);
			AddTimer(0.1, True,, 'DoPop');
		}
	}
}
