		...
		if ((InitialInventory[i].Inventory != None) && (InitialInventory[i].Count > 0))
		{
			...
			for (j=0; j<Min(InitialInventory[i].Count, 199); j++)
			{
				...
				if (inv == None)
				{
					...
					if (Inv == None)
					{
						inv = spawn(InitialInventory[i].Inventory, self,, Location+vect(0,0,20));
					}
...
function bool SetBasedPawnSize(float newRadius, float newHeight)
{
	...
	local vector centerDelta, TCenter, AttemptLocs[32];
...
function GenerateTotalHealth()
{
	...
	if (!bInvincible)
	{
		...
		if ((Health < (HealthChunks[6]/2) || Health <= MinHealth) && (VMP != None) && (VMP.MedkitUseTimer <= 0))
		{
		...
		if ((Health < HealthChunks[6]) && (VMP != None) && (VMP.bDamageGateInTact || (VMP.bDamageGateBullshitFrame && Health < MinHealth-1)))
		{
...
function bool IsProjectileDangerous(DeusExProjectile projectile)
{
	...
	if (projectile.IsA('Cloud'))
	{
		...
		else if ((Projectile.DamageType == 'TearGas' || Projectile.DamageType == 'HalonGas' || Projectile.DamageType == 'OwnedHalonGas') && (Texture == Texture'VisorTex1') && (Mesh == LODMesh'GM_Jumpsuit'))
		{
			bNegated = true;
		}
...
function bool FilterDamageType(Pawn instigatedBy, Vector hitLocation, Vector offset, Name damageType)
{
	...
	if (VMP != None)
	{
		if (!VMP.VMDConfigurePickupDamageFilter(DamageType))
		{
			if (DamageType == 'HalonGas' || DamageType == 'OwnedHalonGas' || DamageType == 'TearGas')
			{
				VMP.bAerosolImmune = True;
			}
			VMP.DrawPickupShield();
			return false;
		}
	}
...
function EHitLocation HandleDamage(int actualDamage, Vector hitLocation, Vector offset, name damageType)
{
	...
	if (VMDBufferPawn(Self) != None)
	{
		VMDBufferPawn(Self).bLastArmorHit = false;
	}
	...
	if (actualDamage > 0)
	{
		if (offset.z > headOffsetZ)		// head
		{
			if (VMDBufferPawn(Self) != None)
			{
			 	if (VMDBufferPawn(Self).bHasHelmet) //Megahack. // || VMDBufferPawn(Self).bHasBodyArmor
			 	{
			  		if (Offset.X > HelmetX)
			  		{
				...
				if (VMDBufferPawn(Self).bHasHeadArmor)
				{
					bHitHelmet = true;
				}
			 	
			 	if ((VMDBufferPawn(Self).bHasHelmet || VMDBufferPawn(Self).bHasHeadArmor) && (bHitHelmet) && (!VMDBufferPawn(Self).BypassesHelmet(damageType)) && (ActualDamage < 25))
			 	{
					VMDBufferPawn(Self).bLastArmorHit = true;
			  		...
			  		ActualDamage = Max(1, ActualDamage * VMDBufferPawn(Self).ArmorStrength);
			  		//return HitPos;
			 	}
				if ((VMDBufferPawn(Self).bClosedSystemHit) && (VMDBufferPlayer(GetPlayerPawn()) != None) && (VMDBufferPlayer(GetPlayerPawn()).HasSkillAugment("TagTeamClosedHeadshot")))
				{
					ActualDamage = int(FMax(ActualDamage+1.0, ActualDamage*1.25));
				}
			}
			...
			else if ((damageType == 'KnockedOut') && (VMDBufferPlayer(GetPlayerPawn()) == None || !VMDBufferPlayer(GetPlayerPawn()).HasSkillAugment("MeleeBatonHeadshots")))
			{
				HealthHead -= actualDamage * 2;
			}
			else
			...
			if (offset.x < 0.0)
			{
				...
				if (VMDBufferPawn(Self) != None)
				{
					VMDBufferPawn(Self).bDamageGateInTact = false;
				}
			...
			else
			{
				if (VMDBufferPawn(Self) != None)
				{
					bHitArmor = true;
			 		...
			 		if ((VMDBufferPawn(Self).bHasBodyArmor) && (bHitArmor) && (!VMDBufferPawn(Self).BypassesHelmet(damageType)) && (ActualDamage < 50))
			 		{
						VMDBufferPawn(Self).bLastArmorHit = true;
			  			VMDBufferPawn(Self).DeflectHelmetProjectile(Max(1, ActualDamage * 0.67), HitLocation, Offset, DamageType);
			  			ActualDamage = Max(1, ActualDamage * VMDBufferPawn(Self).ArmorStrength);
			  			//return HitPos;
			 		}
				}
	...
	if (VMDBufferPawn(Self) != None)
	{
		VMDBufferPawn(Self).bClosedSystemHit = false;
	}
	GenerateTotalHealth();
...
function TakeDamageBase(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType, bool bPlayAnim)
{
	...
	local EHitLocation hitPos;
	local float        shieldMult, ShieldMult2;
	...
	{
		if ((DamageType == 'EMP') && (VMDBufferPawn(Self) != None))
		{
			VMDBufferPawn(Self).VMDSignalDamageTaken(Damage, DamageType, HitLocation, false);
		}
		return;
	}
	...
	if (VMP != None)
	{
		VMP.LastInstigator = InstigatedBy;
		
		VMP.StunLengthModifier = 0;
		if ((VMDBufferPlayer(InstigatedBy) != None) && (VMDBufferPlayer(InstigatedBy).HasSkillAugment("MeleeStunDuration")))
		{
			if (DamageType == 'Stunned')
			{
				VMDBufferPawn(Self).StunLengthModifier = 5;
			}
			else
			{
				VMDBufferPawn(Self).StunLengthModifier = 7.5;
			}
		}
		VMP.LastDamager = InstigatedBy;
	}
	if (actualDamage > 0)
	{
		...
		if (VMP != None)
		{
			ShieldMult2 = VMP.VMDConfigurePickupDamageMult(DamageType);
			VMP.VMDSignalDamageTaken(Damage, DamageType, HitLocation, false);
		...
		if ((DamageType == 'KnockedOut' || DamageType == 'TearGas') && (VMP != None) && (VMP.bMilitantCower))
		{
			VMP.bCower = true;
			VMP.CaptiveEscapeSteps -= Max(float(VMP.StartingHealthValues[6] - Health) / 1.0, 1);
		}
	}
	...
	if ((VMDBufferPlayer(InstigatedBy) != None) && (Animal(Self) == None) && (VMP != None) && (DamageType != 'Autoshot') && (DamageType != 'PoisonEffect') && (!bOnFire || DamageType != 'Burned'))
	{
	 	if (HitPos == HITLOC_HeadFront || HitPos == HITLOC_HeadBack)
	 	{
	  		VMP.PlayPlayerHeadshotSound(VMDBufferPlayer(InstigatedBy));
	 	}
		else if (VMDBufferPawn(Self).bLastArmorHit)
		{
			VMP.TransferArmorHit(VMDBufferPlayer(InstigatedBy));
		}
	}
	...
	if (Health <= 0)
	{
		...
		{
			if ((VMDBufferPlayer(GetPlayerPawn()) != None) && (InstigatedBy != None) && (VMP != None) && (class'VMDStaticFunctions'.Static.TargetIsMayhemable(Self)))
			{
	...
	{
		if ((VMDBufferPawn(Self) != None) && (instigatedBy != None))
		{
			VMDBufferPawn(Self).Burner = instigatedBy;
		}
		CatchFire();
	}
	...
	bPoisonReact = true;
	if (DamageType == 'Poison') bPoisonReact = false;
	
	if (VMP != None)
	{
		if ((DamageType == 'PoisonEffect') && (Health >= VMP.StartingHealthValues[6] * VMP.PoisonReactThreshold))
		{
			bPoisonReact = false;
		}
	}
	...
	{
		TSpeed = 1.0;
		if (HealthLegLeft < 1) TSpeed -= 0.15;
		if (HealthLegRight < 1) TSpeed -= 0.15;
		
		if (TSpeed < 1.0)
		{
			GroundSpeed = Default.GroundSpeed * TSpeed;
		}
	}
...
function GotoDisabledState(name damageType, EHitLocation hitPos)
{
	...
	if ((damageType == 'TearGas' || damageType == 'HalonGas' || DamageType == 'OwnedHalonGas') && (Texture == Texture'VisorTex1') && (Mesh == LODMesh'GM_Jumpsuit'))
		return;
...
function ResetSkinStyle()
{
	...
	for (i=0; i<8; i++)
	{
		...
		if ((VMDBufferPawn(Self) != None) && (VMDBufferPawn(Self).OverhaulTex[i] != None))
		{
			Multiskins[i] = VMDBufferPawn(Self).OverhaulTex[i];
		}
	}
	Skin      = Default.Skin;
...
function EnableCloak(bool bEnable)  // beware! called from C++
{
	...
	VMP = VMDBufferPawn(Self);
	if ((VMP != None) && (VMP.VMDConfigureCloakThresholdMod() > 0))
	{
		CamoCheck = true;
	}
	...
	if (VMDBufferPawn(Self) != None)
	{
		VMDBufferPawn(Self).VMDEnableCloakHook(bEnable);
	}
}
...
function PlayFutzSound()
{
	...
	if ((VMDBufferPawn(Self) != None) && (VMDBufferPawn(Self).LastSmellType ~= "StrongPlayerBloodSmell"))
	{
		return;
	}
...
function PlayFootStep()
{
	...
	if (VMDBufferPawn(Self) != None)
	{
		StepSound = VMDBufferPawn(Self).FootStepSound(Volume, volumeMultiplier);
	}
	else
	{
		stepSound = WalkSound;
	...
	if ((GetPlayerPawn() != None) && (VMDBufferPawn(Self) != None) && (VMDBufferPawn(Self).SendNoiseHeightTolerance > 0))
	{
		PlayRadius *= VMDBufferPawn(Self).ScaleSoundByHeight(VMDBufferPawn(Self).SendNoiseHeightTolerance, GetPlayerPawn());
	}
		...
		if (traceMover != None)
		{
			...
			if (DeusExWeapon(Weapon) != None)
			{
				...
				if (Enemy != None)
				{
					TS = Enemy.Location;
					TE = Location;
					TA = Enemy.Trace(HL, HN, TE, TS, true);
					if (TA != TraceMover)
					{
						TPen = 0;
					}
...
function rotator AdjustAim(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
	...
	if (class'VMDStaticFunctions'.Static.VMDUseDifficultyModifier(Self, "Shoot Explosives")) // Are we fighting the player who is playing on realistic?
	{
...
function bool SwitchToBestWeapon()
{
	...
	while (inv != None)
	{
		...
		if (curWeapon != None)
		{
			...
			if (curWeapon.ReloadCount > 0)
			{
				...
				if (!bValid)
				{
					for (i=0; i<ArrayCount(curWeapon.AmmoNames); i++)
					{
						...
						if ((PossibleAmmo != None) && (PossibleAmmo.AmmoAmount > 0))
						{
							bValid = true;
			...
			if (bValid)
			{
				...
				if (CurWeapon.InvSlotsX > 90) Score += 200000;
				
				//MADDERS: Use grenades only when we have a clear shot!
				if ((CurWeapon.bHandToHand) && (!CurWeapon.bInstantHit) && (!bTraceEnemy)) Score += 1000;
				if ((CurWeapon.bHandToHand) && (EnemyRange > 128)) Score += 1.0;
	...
	if (Weapon != bestWeapon)
	{
		...
		if ((DeusExWeapon(Weapon) != None) && (BestWeapon != None) && (!DeusExWeapon(Weapon).VMDIsMeleeWeapon()) && (DeusExWeapon(Weapon).AmmoType != None) && (DeusExWeapon(Weapon).AmmoType.AmmoAmount <= 0) && (BestWeapon.VMDIsMeleeWeapon()))
		{
			PlayOutOfAmmoSound();
		}
	}
...
function Reloading(DeusExWeapon reloadWeapon, float reloadTime)
{
	...
	if ((reloadWeapon == Weapon) && (ReloadWeapon.ClipCount > 0))
	{
...
State Fleeing
{
	...
	function BeginState()
	{
		...
		if (VMDBufferPawn(Self) != None) VMDBufferPawn(Self).bDoScream = true;
...
State Attacking
{
	...
	function EDestinationType PickDestinationAttacking()
	{
...
RunToRange:
	...
	while ((PickDestinationAttacking() == DEST_NewLocation) && (VMDBufferPawn(Self) == None || VMDBufferPawn(Self).AttackStateLaps < 500))
	{
		if (VMDBufferPawn(Self) != None)
		{
...
ContinueFire:
	while ((!ReadyForWeapon()) && (VMDBufferPawn(Self) == None || VMDBufferPawn(Self).AttackStateLaps < 500))
	{
		...
		if (PickDestinationAttacking() != DEST_SameLocation)
			Goto('RunToRange');
		CheckAttack(true);
		if (!IsWeaponReloading() || bCrouching)
			TurnToward(enemy);
	...
	CheckAttack(true);
	if (PickDestinationAttacking() != DEST_NewLocation)
	{
		if (!IsWeaponReloading() || bCrouching)
			TurnToward(enemy);
		else
...
state AvoidingProjectiles
{
	...
	function bool PickDestination(bool bGotoWatch)
	{
		local NearbyProjectileList projList;
		local bool                 bMove;
		local vector               projVector;
...
Begin:
	...
	if (VMDBufferPawn(Self) != None)
	{
		Sleep(5.0+VMDBufferPawn(Self).StunLengthModifier);
	}
	else
	{
		Sleep(7.5);
