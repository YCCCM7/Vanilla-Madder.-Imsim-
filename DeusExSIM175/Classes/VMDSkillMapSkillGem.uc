...
var localized string TipSpecialized, TipLevel, TipSkillPointCost, TipCurLevelDesc, TipNextLevelDesc, TipTalentPointsLeft;
...
event DrawWindow(GC gc)
{
	...
	if ((TPoints != None) && (bTalentsEnabled))
	{
		...
		GC.DrawStretchedTexture(0+ScaleOffX, 0+ScaleOffY, 32*UnivScale, 32*UnivScale, 0, 0, 32, 32, TPoints);
	}
}
...
function UpdateSkillData()
{
	...
	if ((RelSkill != None) && (LastRef != None))
	{
		...
		{
			SkillPointsLeft = SkillScreen.VMP.SkillPointsAvail;
		...
		MapOffX = 16*(3.0 - (CurDrawScale * ZoomRenderScale));
		MapOffY = 16*(3.0 - (CurDrawScale * ZoomRenderScale));
		SetSize(96, 96);
...
function UpdateHoverTip()
{
	...
	HoverTip = HoverTip$CR()$CR()$TipCurLevelDesc$CR()$BreakUpHoverTip(LevelDescs[CurSkillLevel]);
	if (CurSkillLevel < 3)
	{
		HoverTip = HoverTip$CR()$CR()$TipNextLevelDesc@SprintF(TipSkillPointCost, NextLevelCost)$CR()$BreakUpHoverTip(LevelDescs[CurSkillLevel+1]);
	}
}
