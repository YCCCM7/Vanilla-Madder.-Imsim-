	...
	function BeginState()
	{
		...
		if (VMP != None)
		{
			...
			else
			{
		 	...
		 	VMP.VMDAddToAddiction("Nicotine", 225.0 + (10 * GetAddictionSeed(11)));
		}
		...
		if (P != None)
		{
			...
			if (VMP != None)
			{
				CBA = CigarettesBuffAura(VMP.FindInventoryType(class'CigarettesBuffAura'));
				if (CBA == None)
				{
					CBA = Spawn(class'CigarettesBuffAura');
					if (CBA != None)
					{
						CBA.Frob(VMP, None);
						CBA.Activate();
...
defaultproperties
{
     ...
     largeIconWidth=29
     largeIconHeight=43
     Description="'COUGHING NAILS -- when you've just got to have a cigarette.'|n|nEFFECTS: For 20 seconds: Massively reduced stress levels."
