	...
	if ((VMP != None) && (DragInterfaces[0] != None))
	{
		...
		if (VMP.bSkillAugmentsEnabled)
		{
			for (i=0; i<ArrayCount(MapBranchingFoundation); i++)
			{
				if (MapBranchingFoundationTexture[i] != None)
				{
					...
					MapBranchingFoundation[i].VMDSetupInterfaceVars(256, 256, MapBranchingFoundationTexture[i], Self);
				}
			}
			for (i=0; i<ArrayCount(MapBranching); i++)
			{
		...
		if (VMP.bSkillAugmentsEnabled)
		{
			...
			for (i=0; i<ArrayCount(MapBranching); i++)
			{
				...
				{
					MapBranching[i].VMDSetupBranchInterfaceVars(256, 256, FindTalentGem(MapBranchingTalent[i]), MapBranchingTexture[i], Self);
				}
			}
		}
	}
...
function VMDSkillMapTalentGem FindTalentGem(string SearchTalent)
{
	local int i;
...
defaultproperties
{
     ...
     SkillPointsLeftLabelPos=(X=444,Y=68)
