...
function VMDRunMedkitShellEffects(VMDBufferPlayer VMP)
{
	...
	if (VMP.HasSkillAugment("MedicineStress"))
	{
		...
		PAA = PharmacistArmorAura(VMP.FindInventoryType(class'PharmacistArmorAura'));
		if (PAA == None)
		{
			PAA = Spawn(class'PharmacistArmorAura');
			if (PAA != None)
			{
				PAA.Frob(VMP, None);
				PAA.Activate();
...
state Activated
{
	...
	function BeginState()
	{
		...
		if (VMBP != None)
		{
			...
			VMBP.PlaySound(sound'MedicalHiss', SLOT_None,,, 512);
		}
...
function int VMDConfigureMaxCopies()
{
	...
	if (VMP != None)
	{
		if (VMP.HasSkillAugment("MedicineCapacity"))
		{
			Ret += 5;
		}
		if (VMP.HasSkillAugment("MedicineRevive"))
		{
			Ret += 5;
		}
	}
