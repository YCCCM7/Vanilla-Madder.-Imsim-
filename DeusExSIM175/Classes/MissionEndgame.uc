...
var name TarEndgameConvo; //MADDERS, 11/04/21: Store this so we can execute once flags are here and ready.
...
function Tick(float DT)
{
	...
	if (TarEndgameConvo == 'Barf')
	{
		...
		{
			//MADDERS, 11/04/21: Parse this on the fly, and reset flags AFTER playing the right conversation, NOT before.
			...
			//MADDERS, 11/04/21: Make sure all the flags are deleted, now that flags manager is here.
			DeusExRootWindow(Player.rootWindow).ResetFlags();
		}
	}
}
