...
function VMDApplySpecialDoorStats(optional bool bForce)
{
	...
	if ((VMP != None) && (VMP.HasSkillAugment("LockpickScoutNoise")))
	{
		bDamageRevealed = true;
		if (RevealedDamageThreshold < 1) RevealedDamageThreshold = 1;
	}
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType)
{
	...
	if (bBreakable)
	{
		...
		if ((VMP != None) && (bBreakable) && (RevealedDamageThreshold < MinDamageThreshold))
		{
			...
			if (VMP.HasSkillAugment("LockpickScoutNoise")) DamageAdd *= 2;
			DamageAdd += 1;
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if (!bFrobbable) // || ((bLocked) && (!bHighlight))
		return;
	...
	else if ((bLocked) && (VMDIsSoftDoor()) && (FrobWith == None))
	{
		...
		if ((VMDBufferPlayer(Frobber) == None) || (!VMDBufferPlayer(Frobber).HasSkillAugment("LockpickScoutNoise")))
		{
			AISendEvent('LoudNoise', EAITYPE_Audio, 5.0, 400);
			PlaySound(Sound'TileStep2');
		}
	}
}
