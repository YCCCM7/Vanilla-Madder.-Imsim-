...
var(MEGH) bool bCanHack, bCanHeal, bHasHeal, bHealthBuffed, bHasMedigel, bReversePatrol, bReconMode, bSaidOutOfAmmo;
var(MEGH) string CustomName;

var(MEGHAI) int NumReconTargets;
var(MEGHAI) float ReconScanTimer, LiteHackRefreshTimer, TargetAcquiredSpeechTimer;
...
{
	if ((VMDIsDroneSensitiveMap()) && (!bReconMode))
	{
...
function bool VMDIsDroneSensitiveMap()
{
	local string TName;
	local VMDBufferPlayer VMP;
...
function AddReconTarget(ScriptedPawn NewEnemy)
{
	...
	if (!VMDCanBeMEGHEnemy(NewEnemy)) return;
...
function HealOtherPawn(Pawn TarPawn)
{
	...
	bHasHeal = false;
	TarPawn.PlaySound(sound'MedicalHiss', SLOT_None,,, 768, 1.3);
}
...
function LiteHackHCSecurityComputer(Actor NewComp)
{
	if (NewComp == None || !NewComp.IsA('HCComputerSecurity')) return;
...
state MeghFollowing expands Following
{
	...
	function bool PickDestination()
	{
		...
		{
			if ((VMDPlayerIsOnElevator(DXP)) && (DXP.bDuck > 0))
			{
				if (dist < 32)
				{
...
Done:
	...
	else if ((OrderActor != None) && (OrderActor.IsA('HCComputerSecurity')))
	{
		LiteHackHCSecurityComputer(OrderActor);
	}
...
Done:
	...
	MEGHIssueOrder('MeghFollowing', GetPlayerPawn());
	//GotoState('Standing');
...
function bool FrobDoor(actor Target)
{
	...
	if (DXMover != None)
	{
		...
		if (HitActor == DXMover)
		{
			...
			//DoorWidth = Abs(((DoorBoxMax-DXMover.BasePos) >> ViewRotation).X);
...
function bool AttemptFollowingRegroupTeleport()
{
	...
	{
		if ((HackPlayer != None) && (!PlayerCanSee(HackPlayer, Self)) && (HackPlayer.Region.Zone == None || !HackPlayer.Region.Zone.bWaterZone))
		{
			return SpawnBehindPlayer();
		}
	}
...
function Tick(float DT)
{
	...
	local float TDist, ATDist, TraceDist, TargetX, TargetY, TargetZ;
	...
	if ((!IsInState('Dying')) && (EMPHitPoints > 0))
	{
		...
		bHidden = (HackPlayer.IsInState('Interpolating') || HackPlayer.IsInState('Paralyzed'));
		if (bHidden) return;
		
		if ((IsInState('MeghFollowing')) && (HackPlayer != None) && (!bDoorGhosting))
		{
			if ((Mover(HackPlayer.Base) != None) || (HackPlayer.Region.Zone != None && HackPlayer.Region.Zone.bWaterZone))
			{
				if ((bBlockPlayers) && (Enemy == None))
				{
					BlockReactions(true);
					SetCollision(False, False, False);
				}
			}
			else if (!bBlockPlayers)
			{
				ResetReactions();
		...
		if (LiteHackComputer != None || LiteHackTarget != None)
		{
			...
			if (LiteHackRefreshTimer > 5)
			{
				...
				{
					LiteHackHCSecurityComputer(LiteHackTarget);
				}
				LiteHackRefreshTimer = DT;
			}
		}
		...
		if (TargetX != 0)
		{
			...
			ATDist = Abs(TDist);
			if ((HackPlayer != None) && (Mover(HackPlayer.Base) != None))
			{
				if (ATDist < 32)
				{
					Velocity.X = 0;
		...
		if (TargetZ != 0)
		{
			...
			if ((HackPlayer != None) && (Mover(HackPlayer.Base) != None))
			{
				...
				{
					Velocity.Z = TDist * 5;
					Acceleration.Z = TDist * 5;
				}
				else if (ATDist < 96)
				{
					Velocity.Z = TDist * 10;
					Acceleration.Z = TDist * 10;
				}
				else
				{
					Velocity.Z = TDist * 20;
					Acceleration.Z = TDist * 20;
				}
			}
			else
			{
