//=============================================================================
// ColorThemeMenu_Seawater
//=============================================================================

class ColorThemeMenu_Seawater extends ColorThemeMenu;

/*
   Colors!

	colors(0)  = MenuColor_Background
	colors(1)  = MenuColor_TitleBackground
	colors(2)  = MenuColor_TitleText
	colors(3)  = MenuColor_ButtonFace
	colors(4)  = MenuColor_ButtonTextNormal
	colors(5)  = MenuColor_ButtonTextFocus
	colors(6)  = MenuColor_ButtonTextDisabled
	colors(7)  = MenuColor_HeaderText
	colors(8)  = MenuColor_NormalText
	colors(9)  = MenuColor_ListText
	colors(10) = MenuColor_ListHighlight
	colors(11) = MenuColor_ListFocus

*/

// ----------------------------------------------------------------------
// ----------------------------------------------------------------------

defaultproperties
{
     themeName="Seawater"
     bSystemTheme=True
     Colors(0)=(G=255,B=140)
     Colors(1)=(R=64,G=224,B=192)
     Colors(2)=(G=255,B=255)
     Colors(3)=(G=255,B=255)
     Colors(4)=(R=64,G=255,B=255)
     Colors(5)=(R=255,G=255,B=255)
     Colors(6)=(G=96,B=164)
     Colors(7)=(G=255,B=255)
     Colors(8)=(G=255,B=255)
     Colors(10)=(G=255,B=192)
     Colors(12)=(G=255,B=255)
     
     DefaultColors(0)=(G=255,B=140)
     DefaultColors(1)=(R=64,G=224,B=192)
     DefaultColors(2)=(G=255,B=255)
     DefaultColors(3)=(G=255,B=255)
     DefaultColors(4)=(R=64,G=255,B=255)
     DefaultColors(5)=(R=255,G=255,B=255)
     DefaultColors(6)=(G=96,B=164)
     DefaultColors(7)=(G=255,B=255)
     DefaultColors(8)=(G=255,B=255)
     DefaultColors(10)=(G=255,B=192)
     DefaultColors(12)=(G=255,B=255)
}
