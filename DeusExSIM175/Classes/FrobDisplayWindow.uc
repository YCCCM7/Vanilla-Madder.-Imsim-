...
var localized string msgUnusable, msgBreakableOnly, msgBarrierOnly, msgMinDamageLabel, msgCharacters, msgMysteryLock;
...
function bool ActorIsLit(Actor A)
{
	...
	if (DXRW.HUD == None) return false;
	if (DXRW.HUD.AugDisplay == None) return false;
	...
	if (!FrobCheck)
	{
		if (DXRW.HUD.AugDisplay.bVisionActive)
		{
			VisionCheck = true;
		}
	}
...
function DrawWindow(GC gc)
{
	...
	local bool ExpandHackable, ExpandDoor, FrobCheck; //MADDERS: Draw several rows?
	...
	local int WidthHack, WidthHack2;
	...
	if (player != None)
	{
		...
		if ((frobTarget != None) && (ActorIsLit(FrobTarget))) //FrobCheck || 
		{
			...
			if (frobTarget.IsA('Mover'))
			{
				...
				{
					ExpandDoor = true;
					//if ((DXMover.bLocked) || (DXMover.bBreakable)) ExpandDoor = true;
					if (!dxMover.bFrobbable)
					{
						//if (!DXMover.bBreakable) ExpandDoor = false;
				...
				if (ExpandDoor)
				{
					...
					{
						if (DXMover.RevealedDamageThreshold > 0 || ((VPlayer != None) && (VPlayer.HasSkillAugment("LockpickScoutNoise"))))
						{
					...
					else
					{
						if (device.IsA('SecurityCamera') || device.IsA('AlarmUnit') || (device.IsA('AutoTurretGun')))
						{
							...
							if ((device.IsA('SecurityCamera') && SecurityCamera(device).bActive) || (device.IsA('AlarmUnit') && AlarmUnit(device).bActive) || (device.IsA('AutoTurretGun') && AutoTurret(device.Owner).bActive))
								strInfo = DeusExDecoration(frobTarget).itemName $ ": " $ msgHacked @ msgOn;
							else
								strInfo = DeusExDecoration(frobTarget).itemName $ ": " $ msgHacked @ msgOff;
						}
