...
//---------------
//Sugar, Caffeine, Nicotine, Alcohol, Zyme
var travel float AddictionTimers[5], AfflictionTimers[5], AddictionKickTimers[5],
			AddictionThresholds[5];
var travel float WaterBuildup; //How much water have we drank?
...
var bool bLastSplashWasSelf; //Used for self EMP skill augment, but it has other uses in theory.

var localized string MsgCraving, MsgNewHabit, MsgKickedHabit, MsgHungerReduced, SugarName, CaffeineName, AlcoholName, NicotineName, CrackName;
...
var bool bRollAchieved, bWasFallRoll, bJumpDucked;
var int RollDir;
var float RollCooldown, RollTimer, LastDuckTimer, RollTapMax, RollCapAccel, StoredRollZ, JumpDuckResetTimer, JumpDuckResetTime;
var travel float RollCooldownTimer;
...
//Death negation.
var localized string MsgDeathNegated;
var float NegateDeathCooldown, DamageGateTimer, NegateDeathGateTime;
var int CurTickDamageTaken;

//MADDERS, 4/10/21: Reiterate
var string QueuedBigMessage;
var float BigMessageRetryTime;

//MADDERS, 8/22/23: Plug this shit wherever.
...
//SMELL NODES!
var VMDSmellManager PlayerSmellNode, PlayerAnimalSmellNode, PlayerFoodSmellNode, StrongPlayerFoodSmellNode, PlayerBloodSmellNode, StrongPlayerBloodSmellNode;
var travel float BloodSmellLevel; //How deep in red are we?

var float LastBloodSmellLevel, LastFoodSmellLevel;

//Neat.
var travel int PlayerSeed;

//Mod friendly screen overlay. Neat.
var string VMDVisualOverlayPatch, LastVMDVisualOverlayPatch;
...
//=====================
//NEW GAME DATA
var travel string SelectedCampaign, CampaignNewGameMap, InvokedBindName, AssignedClass, DatalinkID;

//---------------------
var travel bool bAssignedFemale, bDisableFemaleVoice; //Somehow this language has overlapped with reality, but ok
var travel String StoredPlayerSkins[8], StoredPlayerMesh;
var Texture FabricatedSkins[8];
var Mesh FabricatedMesh;
...
var() globalconfig int FaveMaleLensIndex[5], FaveMaleFramesIndex[5], FaveMaleChestIndex[5], FaveMaleCoatIndex[5], FaveMalePantsIndex[5],
			FaveMaleMeshIndex[5], FaveMaleSkinIndex[5],
			FaveFemaleLensIndex[5], FaveFemaleFramesIndex[5], FaveFemaleChestIndex[5], FaveFemaleCoatIndex[5], FaveFemalePantsIndex[5],
			FaveFemaleMeshIndex[5], FaveFemaleSkinIndex[5];
...
var localized string CandyEatenDescs[2], SoyEatenDesc, FishEatenDesc, SodaDrankDescs[4], WaterDrankDesc, CigarettesSmokedDesc,
			...
			FoodSmellDesc[4], BloodSmellDesc[4],
...
var(MADDERSDIFF) travel bool bAdvancedLimbDamageEnabled, bAmmoReductionEnabled, bBossDeathmatchEnabled, bComputerVisibilityEnabled,
bDogJumpEnabled, bDoorNoiseEnabled, bDrawMeleeEnabled, bEnemyDamageGateEnabled, bEnemyDisarmExplosivesEnabled, bEnemyGEPLockEnabled,
bKillswitchHealthEnabled, bLootDeletionEnabled, bLootSwapEnabled, bMayhemSystemEnabled, bRecognizeMovedObjectsEnabled,
bReloadNoiseEnabled, bSaveGateEnabled, bShootExplosivesEnabled, bSmartEnemyWeaponSwapEnabled, bEnemyVisionExtensionEnabled,
bNoticeBumpingEnabled; //, bSeeLaserEnabled

var(MADDERSDIFF) travel int SaveGateCombatThreshold, SavedMayhemForgiveness, SavedGateBreakThreshold, SavedLootSwapSeverity,
				BarfLootReduction,
				BarfCombatThreshold, BarfMayhemForgiveness, BarfGateBreakThreshold, BarfLootSwapSeverity,
				BarfSaveGateTime;
...
var(VMDMayhem) travel int MayhemFactor, OwedMayhemFactor;
...
function VMDPackUpDrones()
{
	...
	if ((TMegh != None) && (VMDShouldPackUpDrone()))
	{
		...
		DroneEMPHitPoints = Max(TMegh.EMPHitPoints, 1);
	...
	if ((bHadMEGH) && (DroneEMPHitPoints > 0))
	{
		...
		if ((MissionNumber == 5) && (FlagBase != None) && (!FlagBase.GetBool('MS_InventoryRemoved')))
		{
			...
			if (TMegh2 != None)
			{
				...
				TMegh2.DroneHealth = DroneHealth;
...
function VMDTriggerDroneAggro(ScriptedPawn TAggro)
{
	...
	if (TMegh != None)
	{
		...
		{
			if (HealthTorso < 50 || HealthHead < 50)
			{
...
function VMDCountMayhemNodes()
{
	...
	{
		if ((VMBP != None) && (IsntPacifistPawn(VMBP)) && (SavedMayhemForgiveness < 0))
		{
...
event PlayerCalcView( out actor ViewActor, out vector CameraLocation, out rotator CameraRotation )
{
	...
	if ((!InConversation()) || ((conPlay != None) && (conPlay.GetDisplayMode() == DM_FirstPerson)))
	{
		...
		else if (bUseDynamicCamera)
		{
			...
			/*if ((HatchetWeapon(InHand) != None) && (HatchetWeapon(InHand).ADSTimer > 0))
			{
				AddGap += 3 * (HatchetWeapon(InHand).ADSTimer / HatchetWeapon(InHand).ADSTime);
			}*/
...
{
	return (bAdvancedLimbDamage || class'VMDStaticFunctions'.Static.VMDUseDifficultyModifier(Self, "Advanced Limb Damage"));
}
...
function bool VMDPlayerIsCloaked()
{
	...
	if (Style == STY_Translucent)
	{
		...
		else if ((AM != None) && (AM.GetAugLevelValue(class'AugCloak') != -1.0)) return true;
...
function bool ShouldAllowHungerAndStress()
{
	...
	if (IsInState('Dying') || IsInState('Paralyzed') || IsInState('Conversation') || IsInState('Interpolating')) return false;
	if (Level.Netmode != NM_Standalone) return false;
	if (ReducedDamageType == 'All' || bHidden) return false;
...
function DeFabricatePlayerAppearance()
{
	...
	if ((IsA('MadIngramPlayer') || IsA('Venom')) && (!bAssignedFemale)) return;
	...
	if (Mesh != None) FabricatedMesh = Mesh;
}
...
function VMDResetPlayerHook(bool bTraining)
{
	...
	//MADDERS: Reset our accessory stuff on NG... Or don't. We have a better method, now.
	//ResetSkillAugments();
...
function VMDSignalSkillAugmentUpdate(string S, bool bUnlock)
{
	local Inventory Inv;
...
function VMDZoneChangeHook(ZoneInfo NewZone)
{
 	...
 	BL = VMDGetBloodLevel();
 	if ((NewZone != None) && (NewZone.bWaterZone) && (BL > 0))
 	{
  		if (BL == 2)
  		{
...
exec function AddSkillPoints(int NewAmount)
{
	...
	SkillPointsTotal += NewAmount;
	SkillPointsAvail += NewAmount;
	
	ClientMessage("Skill points increased by"@NewAmount$"!");
}
...
function VMDProcessDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType)
{
	...
	if ((HeadRegion.Zone.bWaterZone) && (!HasSkillAugment("SwimmingDrowningRate"))) SwimTimer -= Damage / 3;
	...
	if (bStressEnabled)
	{
		VMDModPlayerStress(Damage*0.25, true, 2, true); //*Sqrt(CombatDifficulty)
	}
}
...
function float GetStressFloor()
{
	...
	{
		if (HungerTimer > HungerCap * 0.85) Ret = FClamp(Ret+30.0, -0.1, GetStressCeiling());
...
function float VMDConfigurePickupDamageMult(name DT)
{
	...
	local DeusExWeapon DXW;
 	...
 	for (Inv=Inventory; Inv != None; Inv=Inv.Inventory)
 	{
		...
		{
			Ret *= DXW.VMDConfigurePickupDamageMult(DT);
		}
 	}
...
function bool HasRollObjection(bool bManualRoll)
{
	...
	if (CarriedDecoration != None) return true;
	...
	if (VSize(TVel) < 20) return true;
 	...
 	if (WallMaterial == 'Ladder') return true;
 	HitAct = Trace(HL, HN, Location + (vect(80, 0, 0) >> Rotation), Location, true);
 	if (HitAct != None)
 	{
  		return true;
 	}
...
function InitiatePlayerRoll(Vector InSpeed)
{
	...
	if ((InSpeed << Rotation).X > 0) RollDir = 1;
	else RollDir = -1;
	RollCapAccel = VSize(InSpeed);
	RollTimer = 0.700000;
	if (HasSkillAugment("SwimmingFitness"))
	{
	...
	StoredRollZ = Location.Z;
	PlaySound(Sound'BodyHit',SLOT_Interact,,,96);
}
...
simulated function PlayFootStep()
{
	...
	else
	{
		...
		if (VMDConfigureGroundSpeed() > 1.0) SpeedFactor /= VMDConfigureGroundSpeed();
	}
...
function ProjectHeadshotSound()
{
	...
	{
 		PlaySound(sound'NanoSwordHitFlesh', SLOT_None);
	}
}
...
function RunKillswitchEffects(float DT)
{
 	...
 	else
	{
		...
		if (Frand()*10 < DT)
 		{
  			TakeDamage(5, Self, Location, vect(0,0,0), 'Killswitch');
		}
 	}
}
...
function float VMDConfigureGroundSpeed()
{
	local float TMult, TMath, OAnimFrame;
	local int FitnessLevel;
	local bool bLeftFirst, bRunValidated, FlagRunning;
	...
	//if ((AddictionStates[0] > 0) && (AddictionTimers[0] <= 0)) TMult *= 0.8;
	if ((AddictionStates[1] > 0) && (AddictionTimers[1] <= 0)) TMult *= 0.8;
...
function InflictAddictionWrath(int TIndex)
{
 	...
 	switch(TIndex)
 	{
  		...
  		case 1:
			if (bAddictionEnabled >= 2)
			{
    				GiveCameraKick(240, 240, 180, 1.5, 0.35);
			}
  		break;
  		case 2:
  		break;
  		case 3:
   			GiveCameraKick(320, 320, 240, 2.5, 0.5);
  		break;
  		case 4:
  		break;
 	}
		...
		if (bEnvironmentalSoundsEnabled)
		{
  		...
  		if (StressTimer <= 0)
  		{
			...
			if ((bEnvironmentalSoundsEnabled) && (MusicMode == MUS_Combat || HOL > 0 || AOL > 1))
			{
				...
				LastHeartbeatPitch = 0.95 + (FRand() * 0.1);
			}
...
function bool ShouldSwapFor(string TName, Mesh CurMesh, Texture TSkin)
{
	//MADDERS, 1/9/21: This is too silly to make the current cut... And then some.
	return false;
	
 	switch(CAPS(TName))
 	{
  		case "CRACK":
   		if ((AddictionTimers[4] > 0))
   		{
    			if (TSkin != None || (Fatness != 128)) return True;
    			
    			switch(CurMesh)
    			{
     				case LODMesh'SecurityBot4':
     				case LODMesh'Greasel':
     				case LODMesh'MilitaryBot':
     				case LODMesh'Seagull':
     				case LODMesh'GEPGun3rd':
      					return False;
     				break;
     				default:
      					return True;
  		...
  		break;
  		case "SUGAR":
   		if ((!ShouldSwapFor("Crack", CurMesh, TSkin)) && (AddictionTimers[4] <= 0))
   		{
    			if (AddictionStates[0] > 0)
    			{
     				if ((TSkin != RestoreNormalcy()) || (Fatness < 129)) return True;
  		...
  		case "NORMALITY":
  		default:
    			if (AddictionTimers[4] > 0) return False;
    			if (AddictionStates[0] > 0) return False;
    			
    			if ((TSkin != RestoreNormalcy()) || (Fatness != 128)) return True;
     			switch(CurMesh)
     			{
...
function Mesh ObtainJokeMesh()
{
 	...
 	switch(R)
 	{
  		case 0: return LODMesh'SecurityBot4'; break;
  		case 1: return LODMesh'Greasel'; break;
  		case 2: return LODMesh'Seagull'; break;
  		case 3: return LODMesh'Militarybot'; break;
  		case 4: return LODMesh'GEPGun3rd'; break;
 	}
}
...
function RunAddictionEffects(float DT)
{
	...
	local bool bZymeActive;
 	...
 	//MADDERS: Make us JC Tenton when fat!
 	if (ShouldSwapFor("Sugar", Mesh, Multiskins[0]))
 	{
  		Fatness = 132;
  		if (bAssignedFemale) Mesh = LODMesh'GFM_Trench';
  		else Mesh = LODMesh'GM_Trench_F';
  		Multiskins[0] = RestoreNormalcy();
  		Drawscale = 1.0;
 	}
 	else if (ShouldSwapFor("Crack", Mesh, Multiskins[0]))
 	{
  		Fatness = 128;
  		Mesh = ObtainJokeMesh();
  		if (Mesh == LODMesh'MilitaryBot') Drawscale = 0.35;
  		else Drawscale = 1.0;
  		Multiskins[0] = None;
 	}
 	else if (ShouldSwapFor("Normality", Mesh, Multiskins[0]))
 	{
  		Fatness = 128;
		Mesh = LODMesh'GM_Trench';
 	...
 	for(i=0; i<ArrayCount(AddictionTimers); i++)
 	{
		...
		if ((AddictionKickTimers[i] < 0) && (AddictionStates[i] > 0))
		{
  			CureAddiction(i);
 		}
...
function UpdateSmells()
{
	...
	//MADDERS: Outdated because of lack of specificity,
 	//BL = VMDGetBloodLevel();  	
 	...
 	else if (BloodSmellLevel >= 45*SmellMults[0])
	{
		...
		{
			if (bSmellsEnabled) ClientMessage(BloodSmellDesc[2]);
		}
		BL = 1;
	}
	else if (LastBloodSmellLevel >= 45*SmellMults[0])
	{
		if (bSmellsEnabled) ClientMessage(BloodSmellDesc[0]);
	}
	...
	if (bSmellsEnabled)
	{
 		...
 		else if (FoodScore >= FoodSmellThresholds[0]*SmellMults[1])
		{
			...
			{
				if (bSmellsEnabled) ClientMessage(FoodSmellDesc[2]);
			}
			bFS = true;
		}
		else if (LastFoodSmellLevel >= FoodSmellThresholds[0]*SmellMults[1])
		{
			if (bSmellsEnabled) ClientMessage(FoodSmellDesc[0]);
		}
 	}
...
function UpdateBloodSmell()
{
 	...
 	if ((BloodSmellLevel > 0) && (Region.Zone.bWaterZone))
 	{
...
function VMDTravelPostAcceptHook()
{
	...
	FabricatePlayerAppearance();
	if (IsA('MadIngramPlayer')) bFabricateQueued = true;
	...
	if (ShouldAllowHungerAndStress())
	{
		if (bHungerEnabled)
		{
			...
			if (HUP >= 1.0) ClientMessage(HungerLevelDesc[6]);
			else if (HUP >= 0.75) ClientMessage(HungerLevelDesc[5]);
			else if (HUP >= 0.5) ClientMessage(HungerLevelDesc[3]);
			else if (HUP >= 0.25) ClientMessage(HungerLevelDesc[1]);
...
function VMDSmellManager GenerateSmellNode(string SmellType, Texture NewIcon, int ArraySize, float UpdateTime, float Radius, bool bActive)
{
...
function VMDRunTickHook( float DT )
{
	...
	if (NegateDeathCooldown > 0) NegateDeathCooldown -= DT;
	if (DamageGateTimer > 0) DamageGateTimer -= DT;
	if (GallowsSaveGateTimer > 0) GallowsSaveGateTimer -= DT;
	
	if (BigMessageRetryTime > 0)
	{
		BigMessageRetryTime -= DT;
	}
	...
	if (bFabricateQueued)
	{
 	...
 	else
 	{
		...
		forEach AllActors(class'Actor', TAct)
		{
  		...
  		if (VMDBufferAugmentationManager(AugmentationSystem) == None) ForceAugOverride();
	...
	{
  		HungerTimer += DT * Sqrt(TimerDifficulty);
 		...
 		if (ShouldAllowHungerAndStress())
		{
			...
			if ((HUP >= 1.0) && (LHUP < 1.0)) ClientMessage(HungerLevelDesc[6]);
			else if ((HUP >= 0.75) && (LHUP < 0.75)) ClientMessage(HungerLevelDesc[5]);
			else if ((HUP >= 0.5) && (LHUP < 0.5)) ClientMessage(HungerLevelDesc[3]);
			else if ((HUP >= 0.25) && (LHUP < 0.25)) ClientMessage(HungerLevelDesc[1]);
	...
	{
		if ((Physics != PHYS_Falling) && (WallMaterial != 'Ladder'))
		{
	...
	else
	{
 	...
 	if ((LastDuckTimer < RollTapMax) && (bDuck == 1))
 	{
  		LastDuckTimer += DT;
 	}
	...
	else if (RollCooldownTimer < 0)
	{
		if ((HeadRegion.Zone == None || !HeadRegion.Zone.bWaterZone) && (HasSkillAugment("SwimmingFallRoll")))
		{
			...
			if (bEnvironmentalSoundsEnabled)
			{
				if (!bAssignedFemale)
				{
					PlaySound(Sound'RollCooldownReset',,0.5,,,0.95 + (FRand() * 0.1));
				}
				else
				{
					PlaySound(Sound'RollCooldownResetFemale',,0.7,,,0.95 + (FRand() * 0.1));
				}
			}
		}
		RollCooldownTimer = 0;
...
function VMDAutoSave()
{
	...
	{
    		SaveGame(-3, "AUTO:"@TruePlayerName@"("$TClass$") -"@GetMissionLocation());
	}
	GallowsSaveGateTimer = 0.0;
}

function bool VMDShouldSave(DeusExLevelInfo info)
{
	local string MN;
...
exec function StartNewGamePlus()
{
	local DeusExRootWindow root;
	local VMDMenuSelectAdditionalDifficultyV2 winDiff;
	
	root = DeusExRootWindow(rootWindow);
	if (root != None)
	{
		WinDiff = VMDMenuSelectAdditionalDifficultyV2(root.InvokeMenuScreen(class'VMDMenuSelectAdditionalDifficultyV2', false));
	}
}
