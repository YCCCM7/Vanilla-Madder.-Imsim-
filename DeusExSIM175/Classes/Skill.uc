	...
	{		
		...
		if (localPlayer != None) 
		{
			if (localPlayer.SkillPointsAvail >= GetCost())
			{
				...
				if (VMDBufferPlayer(localPlayer) != None)
				{
					VMDBufferPlayer(localPlayer).AddSkillAugmentPoints(Class, CurrentLevel);
				}
	...
	optional DeusExPlayer usePlayer )
{
	...
	if (CurrentLevel > 0)
	{
		...
		if (VMDBufferPlayer(localPlayer) != None)
		{
			VMDBufferPlayer(localPlayer).RemoveSkillAugmentPoints(Class, CurrentLevel);
		}
...
simulated function bool CanAffordToUpgrade( int skillPointsAvailable )
{
	if ( CurrentLevel == 3 ) 
		return False;
	else if (GetCost() > skillPointsAvailable )
		return False;
	else
		return True;
}
