...
{
	local PlayerPawnExt PPawn;
	
	PPawn = GetPlayerPawn();
	//MADDERS: Advanced key check, and also close us if we DED.
	if (PPawn != None)
	{
		if ((Root != None) && (Root.IsKeyDown(IK_Escape)))
		{
			...
			if (NumbTime < 0)
			{
				...
				if (PPawn.Level != None)
				{
					PPawn.Log("FIXED: Keypad"@keypadOwner@"called emergency closure at"@PPawn.Level.TimeSeconds$"!");
				}
				return;
			}
		}
		else if (NumbTime < 1.0)
		{
			NumbTime = 0.05;
		}
		
		if ((PPawn.IsInState('Dying')) && (Root != None))
		{
			Root.PopWindow();
			return;
		}
...
event bool VirtualKeyPressed(EInputKey key, bool bRepeat)
{
	...
	//MADDERS, 4/13/19: Set numb time to not cause double closures.
	if (IsKeyDown(IK_Escape))
	{
		Numbtime = 0.05;
	}
