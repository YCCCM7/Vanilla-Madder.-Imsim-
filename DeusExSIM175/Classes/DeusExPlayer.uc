...
exec function QuickSave()
{
	...
	VMP = VMDBufferPlayer(Self);
	if ((VMP != None) && (class'VMDStaticFunctions'.Static.UseGallowsSaveGate(Self)))
	{
	...
	switch(QuickSaveNum)
	{
		case 0:
			...
			SaveGame(-1, QuickSaveGameTitle @ "(0)");
			break;
			
		case -1:
			ConsoleCommand("set VMDBufferPlayer QuickSaveNumber " $ -5);
			SaveGame(QuickSaveNum, QuickSaveGameTitle @ "(1)");
			break;
			
		case -5:
			ConsoleCommand("set VMDBufferPlayer QuickSaveNumber " $ -6);
			SaveGame(QuickSaveNum, QuickSaveGameTitle @ "(2)");
			break;
			
		case -6:
			ConsoleCommand("set VMDBufferPlayer QuickSaveNumber " $ -1);
			SaveGame(QuickSaveNum, QuickSaveGameTitle @ "(3)");
			break;
	}
...
simulated function DrugEffects(float deltaTime)
{
	...
	local Rotator rot;
	local DeusExRootWindow root;
	local Texture DrugTex, LoadTex;
	local bool bSway, bFlicker, bMuddle, bDebugDrug;
	local float TTimer, TBase;
	local float AddictionAdd;
	local VMDBufferPlayer VMP;
	...
	if (drugEffectTimer > FMax(30, 30+AddictionAdd)) drugEffectTimer = FMax(30, 30+AddictionAdd);
	
	if (DrugEffectTimer > 0)
	{
         	bSway = True;
         	if (DrugEffectTimer > 10) bFlicker = True;
	 	if (DrugEffectTimer > 20) bMuddle = True;
	}
	...
	if (VMP != None)
	{
		...
		if ((bSway) && (!VMP.bZymeAffected) && (VMP.AddictionStates[3] > 0))
		{
	 	...
	 	if ((VMP.AddictionStates[2] > 0) && (VMP.AddictionTimers[2] <= 0)) bFlicker = True;
	...
	if ((!bMuddle) && (VMP != None) && (root != None))
	{
	 	if (!(VMP.LastVMDVisualOverlayPatch ~= VMP.VMDVisualOverlayPatch))
	 	{
	  		LoadTex = Texture(DynamicLoadObject(VMP.VMDVisualOverlayPatch, class'Texture', True));
	  		if (root.Hud.Background != LoadTex)
	  		{
	   			root.hud.SetBackground(LoadTex);
	...
	if (VMP != None) VMP.LastVMDVisualOverlayPatch = VMP.VMDVisualOverlayPatch;
	
	// random wandering and swaying when drugged
	if ((bFlicker) || (bSway) || (bMuddle))
	{
		if ((root != None) && (root.hud != None) && (bMuddle))
		{			
			DrugTex = Texture'DrunkFX';
			if ((VMP != None) && (VMP.bZymeAffected)) DrugTex = Texture'ZymeDrugEffect';
			
			if ((root.Hud.Background != DrugTex) && (DrugTex != None || LoadTex == None))
			{
				root.hud.SetBackground(DrugTex);
				root.hud.SetBackgroundSmoothing(True);
				root.hud.SetBackgroundStretching(True);
	...
	if (!bMuddle)
	{
		...
		{
			if ((root.hud.background != None) && ((VMP == None) || (CAPS(VMP.VMDVisualOverlayPatch) != CAPS(String(root.hud.background)))))
			{
	...
	if ((DrugEffectTimer <= 10) && (!bFlicker) && (VMP != None) && (!VMP.VMDIsInHandZoomed()))
	{
...
simulated function RefreshSystems(float DeltaTime)
{
	...
	local Repairbot TRep;
   	...
   	if (VMDBufferPlayer(Self) != None)
	{
		VMDBufferPlayer(Self).VMDRunTickHook(deltaTime);
   	}
...
function StartPoison( Pawn poisoner, int Damage )
{
	...
	if ( Level.NetMode != NM_Standalone )
	{
		...
		if (VMDBufferAugmentationManager(AugmentationSystem) != None)
		{
		 	AugLevel = (VMDBufferAugmentationManager(AugmentationSystem).VMDConfigureDamageMult('Poison'));
...
function Landed(vector HitNormal)
{
	...
	if ((Velocity.Z < -1.4 * JumpZ) && (VMDBufferPlayer(Self) != None) && (VMDBufferPlayer(Self).HasSkillAugment("SwimmingFallRoll")))
	{
		...
		{
			VMP.InitiatePlayerRoll(Velocity * vect(1.5, 1.5, 0));
			VMP.bWasFallRoll = true;
		}
	}
	//Rolling reduces fall damage by ~20%
	RollRed = 1.0;
	if ((VMP != None) && (VMP.RollTimer > 0.0)) RollRed = 0.8;
...
function int HealPlayer(int baseHealPoints, optional Bool bUseMedicineSkill)
{
	...
	if (adjustedHealAmount > 0)
	{
		if (bUseMedicineSkill)
		{
			if ((VMDBufferPlayer(Self) != None) && (HeadRegion.Zone.bWaterZone)) PlaySound(sound'MedicalHiss', SLOT_None,,, 256, 0.7);
...
function DoJump( optional float F )
{
	...
	local float scaleFactor, augLevel, UsePitch;
	local VMDBufferPlayer VMP;
	...
	VMP = VMDBufferPlayer(Self);
	if ((VMP != None) && (!VMP.HasRollObjection(true)) && (VMP.LastDuckTimer < VMP.RollTapMax) && (VMP.HasSkillAugment("SwimmingRoll")))
	{
		VMDBufferPlayer(Self).InitiatePlayerRoll(Normal(Velocity)*(GetCurrentGroundSpeed()*1.5));
		return;
	}
...
state PlayerWalking
{
	...
	function ProcessMove ( float DeltaTime, vector newAccel, eDodgeDir DodgeMove, rotator DeltaRot)
	{
		...
		if (VMP != None)
		{
			...
			if (VMP.HasSkillAugment("SwimmingFitness") || WallMaterial == 'Ladder')
			{
				bCanJumpDuck = true;
			...
			if (VMP.RollTimer > 0)
			{
				...
				if ((!VMP.bWasFallRoll) && (Location.Z > VMP.StoredRollZ))
				{
					VMP.RollTimer = 0;
					VMP.StoredRollZ = 0.0;
				}
		...
		if (bIsCrouching || bForceDuck || bJumpDuck)
		{
			...
			if ((bOnFire) && (VMDBufferPlayer(Self) != None) && (VMDBufferPlayer(Self).HasSkillAugment("HeavyDropAndRoll")) && (VSize(Velocity) > 60))
			{
				if (Abs(Rotator(NewAccel).Yaw - VMDBufferPlayer(Self).LastFireRollDir) > 24000)
				{
					burnTimer -= 1.5;
					if (BurnTimer <= 0) ExtinguishFire();
				}
				VMDBufferPlayer(Self).LastFireRollDir = Rotator(NewAccel).Yaw;
			}
		...
		else if ((WallMaterial != 'Ladder') && (Physics != PHYS_Falling))
		{
		...
		if ((bCantStandUp) || ((VMP != None) && (VMP.RollTimer > 0)))
			bForceDuck = True;
		else
			bForceDuck = False;
...
*/
		...
		{
			if ((VMDBufferPlayer(Self) == None) && (DeusExWeapon(Weapon).GetWeaponSkill() > -0.25))
			{
				bIsWalking = True;
				newSpeed = defSpeed;
			}
			else
			{
				...
				NewSpeed = VMDBufferPlayer(Self).VMDConfigureWeaponMassSpeed(NewSpeed, DefSpeed, DeusExWeapon(Weapon));
				if (NewSpeed <= 0)
				{
					NewSpeed = DefSpeed;
					bIsWalking = true;
		...
		if (VMDBufferPlayer(Self) != None)
		{
		 	if ((VMDBufferPlayer(Self).AddictionStates[4] > 0) && (VMDBufferPlayer(Self).AddictionTimers[4] <= 0))
		 	{
		  		bInverse = True;
		 	}
		 	//Past 23 hours, invert our controls.
		 	//92*60 = 5520
		 	if ((VMDBufferPlayer(Self).bKillswitchEngaged) && (VMDBufferPlayer(Self).KillswitchTime > 5520))
		 	{
		  		bInverse = True;
		 	}
		 	//MADDERS: During tasing, flip our shit around per half second.
		 	if (VMDBufferPlayer(Self).TaseDuration > 0)
		 	{
		  		if (VMDBufferPlayer(Self).TaseDuration % 1.0 > 0.5) bInverse = !bInverse;
		 	}
		...
		if (VMDBufferPlayer(Self) != None) VMDBufferPlayer(Self).VMDProcessMoveHook(deltaTime);
...
event HeadZoneChange(ZoneInfo newHeadZone)
{
	...
	if ((newHeadZone.bWaterZone) && (!HeadRegion.Zone.bWaterZone))
	{
		...
		if (SwimTimer > SwimDuration || SwimTimer <= 0.0) swimTimer = swimDuration;
...
simulated event RenderOverlays( canvas Canvas )
{
	...
	if (Human(Self).bRadarCloaked)
	{
		Fatness = Rand(6) + 125;
		if (inHand != None)
			inHand.Fatness = Rand(4) + 126;
	}
	else
	{
		Fatness = Default.Fatness;
		if (inHand != None)
			inHand.Fatness = Default.Fatness;
	}
...
{
	local bool bCanPickup, FlagNull;
			...
			if (!FlagRotationWorked)
			{
				...
				if (VMP != None)
				{
					if (DXW != None)
					{
						...
						if (DXW.AmmoName != None) FindA = DeusExAmmo(FindInventoryType(DXW.AmmoName));
...
function DoFrob(Actor Frobber, Inventory frobWith)
{
	...
	{
		//MADDERS: Insert super futz for super owned later.
...
exec function PutInHand(optional Inventory inv)
{
	...
	/*VMP = VMDBufferPlayer(Self);
	if ((VMP != None) && (VMP.VMDPlayerIsCrafting(false)))
	{
		return;
	}*/
...
function GrabDecoration()
{
	...
	if (carriedDecoration == None)
	{
		if ((FrobTarget != None) && FrobTarget.IsA('Decoration') && (Weapon == None))
		{
			...
			{
				if ((VMDBufferDeco(FrobTarget) != None) && (VMDBufferDeco(FrobTarget).RejectPickup()))
				{
				 return;
				}
...
exec function bool DropItem(optional Inventory inv, optional bool bDrop)
{
	...
	if (item != None)
	{
		...
		// if we are highlighting something, try to place the object on the target
		if ((FrobTarget != None) && (Item != None) && (!item.IsA('POVCorpse')))
		{
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType)
{
	...
	local int i, TDamage;
	local float ModMult, PickupMult;
	local Medkit Kit;
	local VMDBufferPlayer VMP;
	...
	if (VMP != None)
	{
		...
		if (VMP.DamageGateTimer > 0 || ((VMP.CurTickDamageTaken >= 95*ModMult) && (DamageType == 'Shot' || DamageType == 'AutoShot')))
		{
			Damage = 0;
		}
	...
	//Holy fucking hack, batman! ~MADDERS
	if ((VMP != None) && (Damage > 15))
	{
		if ((damageType == 'EMP') && (!bEMPImmunity))
		{
	 		VMP.HUDEMPTimer += ActualDamage / 3;
	 		ShowHUD(False);
		}
	}
	...
	{
	 	VMP.TaseDuration += FMax(2.0, float(ActualDamage)/3.0);
	}
	...
	if ((DamageType == 'TearGas' || DamageType == 'HalonGas') && (Damage > 0))
	{
		if (VMP != None)
		{
			PickupMult = VMP.VMDConfigurePickupDamageMult(DamageType);
			if (PickupMult <= 0)
			{
				bPickupBlock = true;
			}
	...
	if (( Level.NetMode == NM_DedicatedServer ) || ( Level.NetMode == NM_ListenServer ))
	{
		...
		if (MPHitLoc == 0)
		{
			...
			if (VMDBufferPlayer(Self) != None)
			{
				VMDBufferPlayer(Self).bLastSplashWasSelf = false;
			}
			return;
		}
	...
	else // Normal damage code path for single player
	{
		if (offset.z > headOffsetZ)		// head
		{
			...
			{
				//----------------
				//MADDERS: Cap damage off (again) with a damage gate. Do this here again beacuse x2 tho.
				if (DamageType == 'Shot' || DamageType == 'AutoShot')
				{
					if (VMP != None)
					{
						actualDamage = Clamp(ActualDamage*2, 0, (95*ModMult)-VMP.CurTickDamageTaken);
						VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + ActualDamage, 0, 95*ModMult);
					}
					else
					{
		...
		else if (offset.z < 0.0)	// legs
		{
			if (offset.y > 0.0)
			{
				...
				if ((DamageType == 'Shot' || DamageType == 'AutoShot') && (VMP != None))
				{
					actualDamage = Clamp(ActualDamage*2, 0, (190*ModMult)-VMP.CurTickDamageTaken);
					VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + ActualDamage, 0, 95*ModMult);
				}
			...
			{
				TDamage = Clamp(-HealthLegRight, 0, (95*ModMult)-VMP.CurTickDamageTaken);
				VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + TDamage, 0, 95*ModMult);
				HealthLegLeft -= TDamage;
				HealthLegRight = 0;
			}
			else if ((HealthLegLeft < 0) && (HealthLegRight > 0))
			{
				TDamage = Clamp(-HealthLegLeft, 0, (95*ModMult)-VMP.CurTickDamageTaken);
				VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + TDamage, 0, 95*ModMult);
		...
		else						// arms and torso
		{
			...
			if (HealthArmLeft < 0)
			{
				TDamage = Clamp(-HealthArmLeft, 0, (95*ModMult)-VMP.CurTickDamageTaken);
				VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + TDamage, 0, 95*ModMult);
				HealthTorso -= TDamage;
				HealthArmLeft = 0;
			}
			if (HealthArmRight < 0)
			{
				TDamage = Clamp(-HealthArmRight, 0, (95*ModMult)-VMP.CurTickDamageTaken);
				VMP.CurTickDamageTaken = Clamp(VMP.CurTickDamageTaken + TDamage, 0, 95*ModMult);
				HealthTorso -= TDamage;
				HealthArmRight = 0;
			}
	...
	if ((CarriedDecoration != None) && (DamageType != 'Hunger'))
		DropDecoration();
...
function bool DXReduceDamage(int Damage, name damageType, vector hitLocation, out int adjustedDamage, bool bCheckOnly)
{
	...
	local bool bReduced, bRollReduction, bPickupBlock;
	
	bReduced = False;
	newDamage = Float(Damage);
	
	//MADDERS: All sorts of shit going on with configuring damage now.
	//TL;DR: Step 1. Configure damage with augs
	//Signal that damage taken with augs.
	//Step 2: If damage > 0 now, configure with items
	//Signal that damage taken with items.
	AugLevel = 1.0;
	if (VMDBufferAugmentationManager(AugmentationSystem) != None)
	{
		AugLevel = (VMDBufferAugmentationManager(AugmentationSystem).VMDConfigureDamageMult(damageType));
	...
	if (VMDBufferPlayer(Self) != None)
	{
		SkillLevel = VMDBufferPlayer(Self).VMDConfigurePickupDamageMult(DamageType);
		if (SkillLevel == 0)
		{
			bPickupBlock = true;
		}
		VMDBufferPlayer(Self).VMDSignalDamageTaken(Damage, DamageType, HitLocation, bCheckOnly);
	}
	...
	if ((VMDBufferPlayer(Self) != None) && (VMDBufferPlayer(Self).RollTimer > 0) && (VMDBufferPlayer(Self).HasSkillAugment("SwimmingFitness")))
	{
		bRollReduction = true;
	}
	...
	if (CombatDifficulty < 1.0)
	{
		...
		if ((newDamage <= 1) && (Damage > 0) && (!bPickupBlock))
			newDamage = 1;
	}
	...
	switch(DamageType)
	{
		case 'Shot':
		case 'Autoshot':
			if (CombatDifficulty >= 1.0) newDamage *= CombatDifficulty;
			
			//MADDERS: Establish a damage gate for not hitting for >95 damage per hit.
			if (NewDamage > 95) NewDamage = 95;
