	...
	while(anAug != None)
	{
		...
		if (anAug != augBoosting)
		{
			if (bBoostEnabled)
			{
				...
				if ((anAug.bIsActive) && (!anAug.bBoosted) && (anAug.CurrentLevel < anAug.MaxLevel) && (AugDrone(AnAug) == None))
				{
					anAug.Deactivate();
					anAug.CurrentLevel++;
					anAug.bBoosted = True;
...
function ActivateAll()
{
	...
	if ((Player != None) && (Player.IsInState('Paralyzed') || Player.IsInState('Dying')))
	{
		return;
	}
