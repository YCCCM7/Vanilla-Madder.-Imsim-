...
var VMDSkillAugmentPageFlipper EnterSkillAugments;
var PersonaTitleTextWindow SkillAugmentLabel;
var localized String SkillAugmentLabelText;
...
function CreateSkillsTileWindow()
{
	...
	winTile.SetSize(302, 297); //MAKE BIGGER! +27
...
function CreateButtons()
{
	...
	winActionButtons.SetPos(10, 338); //+27
	winActionButtons.SetWidth(149);
	winActionButtons.FillAllSpace(False);
	
	SkillAugmentLabel = PersonaTitleTextWindow(NewChild(class'PersonaTitleTextWindow'));
	SkillAugmentLabel.SetPos(528, 324); //Used to be 504.
	
	if ((VMDBufferPlayer(GetPlayerPawn()) != None) && (VMDBufferPlayer(GetPlayerPawn()).bSkillAugmentsEnabled) && (!VMDHasSkillAugmentObjection()))
	{
		SkillAugmentLabel.SetText(SkillAugmentLabelText);
		EnterSkillAugments = VMDSkillAugmentPageFlipper(winClient.NewChild(class'VMDSkillAugmentPageFlipper'));
		
		//MADDERS: Use a more formal placement now.
		//EnterSkillAugments.SetPos(325, 147);
		EnterSkillAugments.SetPos(580, 243);
		EnterSkillAugments.SetSkillPage(True, True, 0);
	}
...
function CreateSkillPointsWindow()
{
	...
	winSkillPoints.SetPos(250, 341); //+27
...
function bool ButtonActivated( Window buttonPressed )
{
	...
	if ((ButtonPressed != None) && (buttonPressed.IsA('PersonaSkillButtonWindow')))
	{
...
function UpgradeSkill()
{
	...
	// Transcended - Apply bonuses immediately if swimming
	if ((Player != None) && (selectedSkill.IsA('SkillSwimming')) && (player.IsInState('PlayerSwimming')))
	{
		player.swimDuration = class'VMDStaticFunctions'.Static.GetPlayerSwimDuration(Player);
		player.swimTimer = (player.swimDuration - player.swimTimer);
		player.WaterSpeed = class'VMDStaticFunctions'.Static.GetPlayerWaterSpeed(Player);
	}
...
{
 	local PersonaSkillButtonWindow S;
 	
 	forEach AllObjects(class'PersonaSkillButtonWindow', S)
 	{
		S.RefreshSkillInfo();
 	}
}
...
function bool VMDHasSkillAugmentObjection()
{
	if (IsBurden() || IsCassandra()) return true;
