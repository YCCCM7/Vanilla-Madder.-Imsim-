...
event DestroyWindow()
{
	...
	if ((compOwner.IsA('Computers')) && (compOwner != None))
	{
		...
		// Keep track of the last time this computer was hacked
      		...
      		{
         		if (bHacked)
            			player.SetComputerHackTime(Computers(compOwner),30, player.level.TimeSeconds); //player.level.TimeSeconds
	...
	else if (compOwner.IsA('ATM'))
	{
		// Keep track of the last time this computer was hacked
		if (bHacked)
			ATM(compOwner).lastHackTime = 30; //player.Level.TimeSeconds;
...
function CreateHackWindow()
{
	...
	if ((skillLevel > 0) && (bUsesHackWindow))
	{
		...
		if (DetectionTime < 8.0) DetectionTime = 8.0;
...
function HackDetected(optional bool bDamageOnly)
{
	if (compOwner.IsA('Computers'))
	{
		...
		Computers(CompOwner).LockoutTime = Computers(compOwner).lockoutDelay / player.SkillSystem.GetSkillLevelValue(class'SkillComputer');
		//Computers(compOwner).lockoutTime = player.Level.TimeSeconds;
	}
	else if (CompOwner.IsA('ATM'))
	{
		ATM(compOwner).bLockedOut = True;
		ATM(CompOwner).LockoutTime = ATM(compOwner).lockoutDelay / player.SkillSystem.GetSkillLevelValue(class'SkillComputer');
