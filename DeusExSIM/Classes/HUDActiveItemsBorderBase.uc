...
function AddIcon(Texture newIcon, Object saveObject)
{
	...
	if (++iconCount == 1)
		Show();
	WinIcons.SetHeight((Class'HUDActiveItemBase'.Default.IconHeight*IconCount) + ((IconCount-1)*2));
...
{
	local Window nextWindow, currentWindow;
	...
	while(currentWindow != None)
	{
		...
		if (currentWindow.GetClientObject() == removeObject)
		{
			...
			{
				WinIcons.SetHeight(Class'HUDActiveItemBase'.Default.IconHeight*IconCount);
			}
