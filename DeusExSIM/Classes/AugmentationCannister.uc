...
simulated function bool UpdateInfo(Object winObject)
{
	...
	for(canIndex=0; canIndex<ArrayCount(AddAugs); canIndex++)
	{
		...
		if (aug != None)
		{
			...
			//MADDERS, 12/13/23: Include our advanced description, thank you.
	...
	winInfo.AppendText(winInfo.CR() $ DirtyLineBreak $ winInfo.CR());
...
function ProcessVMDChanges()
{
	...
	if (bDidSetup) return;
 	if (Level.TimeSeconds > 5) return;
	
	Super.ProcessVMDChanges();
	
	MapName = class'VMDStaticFunctions'.Static.VMDGetMapName(Self);
	//MADDERS, 6/24/23: Flip these two around, as they now have inverted install locations.
	//8/9/23: Stop swapping for omega.
	if ((Owner == None) && (MapName != "50_OMEGASTART"))
	{
