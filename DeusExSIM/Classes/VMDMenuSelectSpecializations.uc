...
function CreateInfoWindows()
{
	...
	for (i=0; i<2; i++)
	{
		...
		//if (i == 0) DescBoxes[i].SetWindowAlignments(HALIGN_Right, VALIGN_Top);
...
function CreatePreviewLists()
{
	...
	for (i=0; i<ArrayCount(SpecializationLists); i++)
	{
		...
		//if (i == 0) SpecializationLists[i].SetWindowAlignments(HALIGN_Right, VALIGN_Top);
		...
		if (j < Min(ArrayCount(PreviewNames), ArrayCount(SpecializationDescs)))
		{
			...
			if (SpecializationLists[i] != None)
			{
				if (CurPresetIndex == 0)
				{
					...
					UpdateListColors(SpecializationLists[i], False);
					//SpecializationLists[i].Show(True);
				}
				else
				{
					SpecializationLists[i].SetSensitivity(False);
					UpdateListColors(SpecializationLists[i], True);
...
function UpdateListColors(MenuUIListWindow TList, bool bDarken)
{
	local int ColIndex;
	local ColorTheme Theme;
	
	if ((Player != None) && (Player.ThemeManager != None))
	{
		Theme = Player.ThemeManager.GetCurrentMenuColorTheme();
	}
	...
	if (bDarken)
	{
		TList.ColText = VMDNerfColor(Theme.GetColorFromName('MenuColor_ListText'));
		TList.ColTextHighlight = VMDNerfColor(Theme.GetColorFromName('MenuColor_ListTextHighlight'));
		TList.colHighlight = VMDNerfColor(theme.GetColorFromName('MenuColor_ListHighlight'));
		TList.colFocus = VMDNerfColor(theme.GetColorFromName('MenuColor_ListFocus'));
	}
	else
	{
		TList.ColText = Theme.GetColorFromName('MenuColor_ListText');
		TList.ColTextHighlight = Theme.GetColorFromName('MenuColor_ListTextHighlight');
		TList.colHighlight = theme.GetColorFromName('MenuColor_ListHighlight');
		TList.colFocus = theme.GetColorFromName('MenuColor_ListFocus');
	}
	
	TList.SetTextColor(TList.ColText);
	TList.SetHighlightTextColor(TList.ColTextHighlight);
	TList.SetHighlightColor(TList.ColHighlight);
	TList.SetFocusColor(TList.ColFocus);
	
	// Loop through columns, setting text color
	for (ColIndex=0; ColIndex<TList.GetNumColumns(); ColIndex++)
	{
		TList.SetColumnColor(ColIndex, TList.ColText);
	}
}

//MADDERS, 8/9/23: Don't got no dark list color. Fuck it. Use this instead.
function Color VMDNerfColor(Color StartColor)
{
	local Color Ret;
