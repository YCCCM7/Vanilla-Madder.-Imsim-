...
// HUDObjectSlot
//=============================================================================
class HUDObjectSlot extends ToggleWindow;
...
//MADDERS: For skilled tools.
var localized string AmpLabel;
...
function UpdateItemText()
{
	...
	if (item != None)
	{
		if (item.IsA('DeusExWeapon'))
		{
			...
			//MADDERS, 5/24/23: Improved belt text here as well.
			if ((!Weapon.VMDHasJankyAmmo()) && (weapon.AmmoType != None))
			...
			//MADDERS, 5/24/23: Improved belt text, all around.
			if ((Weapon.VMDHasJankyAmmo()) && (Weapon.ReloadCount > 0))
			{
				if (weapon.AmmoType.AmmoAmount > 0) //MADDERS, 5/24/23: Used to be 1, not 0.
		...
		else if (item.IsA('DeusExPickup') && (!item.IsA('NanoKeyRing')))
		{
			...
			{
				//MADDERS: Amp system is gone and dead. Just do things normally now.
				/*if (SkilledTool(Item) != None)
				{
				 //MADDERS: Let maximum amp percentage roll over.
				 //VMDConfigureMaxCopies()-1
				 AmpValue = float(DeusExPickup(Item).NumCopies-1) * (100.0/(DeusExPickup(Item).MaxCopies-1));
				 itemText = AmpLabel @ int(AmpValue)$"%";
				}
				else*/
...
event DrawWindow(GC gc)
{
	...
	if ((item != None) && (item.Icon != None) && (!bDragging))
	{
		// Draw the icon
      		DrawHUDIcon(gc);
      		DrawHUDModes(gc); //MADDERS: Draw our mode display!
		...
		gc.DrawText(2, 42, 42, 7, item.BeltDescription);
   	...
   	else if ((item == None) && (player != None) && (player.Level.NetMode != NM_Standalone) && (player.bBeltIsMPInventory))
   	{
		...
		if ((objectNum >= 1) && (objectNum <= 3))
      		{
         		gc.DrawText(2, 42, 42, 7, "WEAPONS");
      		}
      		else if ((objectNum >= 4) && (objectNum <= 6))
      		{
         		gc.DrawText(2, 42, 42, 7, "GRENADES");
      		}
      		else if ( ((objectNum >= 7) && (objectNum <= 9)) || (objectNum == 0) )
      		{
         		gc.DrawText(2, 42, 42, 7, "TOOLS");
      		}
   	}
...
function DrawHUDModes(GC gc)
{
	...
	//MADDERS: Moved to drawing on the ammo display.
	return;
	...
	if (DXW != None)
	{
		...
		if (T != None)
		{
		 	gc.SetStyle(DSTY_Translucent);
		 	gc.SetTileColorRGB(255, 255, 255);
		 	gc.DrawTexture(8, 51, 16, 16, 0, 0, T);
		}
	}
}
...
function RemoteUseContents()
{
	...
	if ((WinInv != None) && (ItemButton != None))
	{
		WinInv.SelectInventory(ItemButton);
		if (DeusExWeapon(Item) != None || SkilledTool(Item) != None || FireExtinguisher(Item) != None)
		{
...
function PersonaScreenInventory GetInvScreen()
{
	local PersonaScreenInventory Ret;
	
	forEach AllObjects(class'PersonaScreenInventory', Ret) break;
	
	return Ret;
}

function PersonaInventoryItemButton GetItemButton()
{
	local PersonaInventoryItemButton Ret;
	
	if (Item == None) return Ret;
	
	forEach AllObjects(class'PersonaInventoryItemButton', Ret)
	{
		if (Ret.GetClientObject() == Item)
		{
			break;
		}
	}
