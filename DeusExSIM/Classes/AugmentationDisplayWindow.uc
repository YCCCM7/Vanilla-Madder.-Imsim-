...
var Color	colRed, colGreen, colYellow, colHiddenDoor, colWhite;

//MADDERS, 2/22/21: New DXT ports.
var localized String msgDead;
var localized String msgEMPHealth;
var localized String msgEnergyPercent;
var localized String msgRebooting;
var localized String msgWearing;

//MADDERS, 4/23/22: Barf. Use this for alt vision bullshit.
function bool VMDUseAlternateVision()
{
...
function Actor TraceLOS(float checkDist, out vector HitLocation)
{
	...
	foreach Player.TraceActors(class'Actor', target, HitLoc, HitNormal, EndTrace, StartTrace)
	{
		...
		FlagTarget = false;
		if (Target.IsA('Pawn') || Target.IsA('ThrownProjectile') || target.IsA('ThrownProjectile')) FlagTarget = true;
		else if ((target.IsA('DeusExDecoration')) && (!target.IsA('Trash'))) FlagTarget = true;
		else if ((target.IsA('Inventory')) && (Inventory(target).bProjTarget)) FlagTarget = true;
		else if ((target.IsA('DeusExMover')) && (DeusExMover(target).bBreakable)) FlagTarget = true;
		else if ((Target.IsA('Mover')) && (Target.BlendTweenRate[0] > 0.0)) FlagTarget = true;
...
function UpdateWinDrone()
{
	if (winDrone != None)
	{
		if ((VMDBufferPlayer(Player) != None) && (VMDBufferPlayer(Player).bAltSpyDroneView))
		{
			winDrone.SetViewportActor(Player); // New mode, player view is spydrone view while small window is player view.
		}
		else
		{
			winDrone.SetViewportActor(Player.aDrone); // Original mode, small window is the spydrone view.
		}
	}
}
...
function PostDrawWindow(GC gc)
{
   	...
   	if ((pp != None) && (pp.bShowScores) && (DeusExGameInfo(Player.DXGame) != None))
	{
		...
		else if ((DeusExGameInfo(Player.DXGame).bIsTeamGame()))
		{
...
function DrawSpyDroneAugmentation(GC gc)
{
	...
	if (winDrone != None)
	{
		...
		if ((Player.Energy / Player.EnergyMax) < 0.2)
		{
			str = msgEnergyLow;
			gc.GetTextExtent(0, w, h, str);
			x = boxCX - w/2;
...
function GetTargetReticleColor( Actor target, out Color xcolor )
{
	...
	if ((Player != None) && (DeusExGameInfo(Player.DXGame) != None))
	{
		bDM = (!DeusExGameInfo(Player.DXGame).bIsTeamGame());
		bTeamDM = (DeusExGameInfo(Player.DXGame).bIsTeamGame());
	}
	
	if ( target.IsA('ScriptedPawn') )
	{
		if (Target.ScaleGlow < 0.02 || Target.AIGETLIGHTLEVEL(Target.Location)*Target.ScaleGlow <= 0.005)
		{
			xcolor = colWhite;
		}
		else
		{
			if (ScriptedPawn(target).GetPawnAllianceType(Player) == ALLIANCE_Hostile)
				xcolor = colRed;
			else if (ScriptedPawn(target).GetPawnAllianceType(Player) == ALLIANCE_Neutral)
				xcolor = colYellow;
			else
				xcolor = colGreen;
		}
	...
	else if (target.IsA('Mover'))
	{
		...
		{
			if ((DeusExMover(Target).bHighlight) && (!DeusExMover(Target).bBreakable)) xcolor = colHiddenDoor;
		}
	}
	...
	if ((VMP != None) && (VMP.HUDScramblerTimer > 0))
	{
		if (VMP.IsEMPFlicker(VMP.HUDScramblerTimer+0.1))
		{
			if (XColor == ColRed) XColor = ColGreen;
			else if (XColor == ColGreen) XColor = ColRed;
		}
	}
}
...
{
	local String str, EMPStr, EnergyStr;
	local Actor target;
	local float boxCX, boxCY, boxTLX, boxTLY, boxBRX, boxBRY, boxW, boxH;
	local float x, y, w, h, mult, EMPMult, EnergyMult;
	local Vector v1, v2;
	local int i, j, k;
	...
	if (target != None)
		closeTarget = TraceLOS(160,AimLocation); // A close scripted pawn that will react to you pointing a weapon at them
	
   	targetplayerhealthstring = "";
   	targetplayerlocationstring = "";
	
	//MADDERS: Set these to true unless proven otherwise.
	//Notes: Only show lit actors and/or known doors.
	bLightValid = true;
	bDoorValid = true;
	if (Target != None)
	{
		bLightValid = (Mover(Target) != None || (Target.AIGETLIGHTLEVEL(Target.Location)*Target.ScaleGlow > 0.005 && Target.ScaleGlow >= 0.02) || bool(Target.GetPropertyText("bEverNotFrobbed")) == false);
		if (DeusExMover(Target) != None)
		{
			bDoorValid = (DeusExMover(Target).bHighlight);
		}
		else if (Target.IsA('CBreakableGlass'))
		{
			BarfBreakable = bool(Target.GetPropertyText("bBreakable"));
			BarfRevealed = Target.bIsSecretGoal;
			BarfOrigStrength = Target.BlendTweenRate[0];
			BarfStrength = float(Target.GetPropertyText("DoorStrength"));
			BarfThreshold = int(Target.GetPropertyText("MinDamageThreshold"));
			
			if ((BarfRevealed) && (BarfStrength < BarfOrigStrength)) BarfDamaged = true;
			
			if (BarfDamaged) BarfDamRevealed = true;
			//if ((VPlayer != None) && (VPlayer.HasSkillAugment('LockpickScoutNoise'))) BarfDamRevealed = true;
			
			if (BarfOrigStrength < 0.1)
			{
				BarfDamaged = false;
				BarfDamRevealed = false;
			}
	...
	if ((target != None) && (bLightValid) && (bDoorValid))
	{
		...
		pawn = ScriptedPawn(closeTarget);
		
		// Transcended - If I can see the player pointing their weapon at me, tell them off! (But only if I'm not running to talk to them)
		if ((pawn != None) && (Weapon != None) && (pawn.GetPawnAllianceType(Player) != ALLIANCE_Hostile) && (!pawn.IsInState('WaitingFor')) && (!pawn.IsInState('RunningTo')) && (!pawn.IsInState('GoingTo')) && (!pawn.IsInState('FirstPersonConversation')) && (!pawn.IsInState('Fleeing')) && (Human(Player).bAIReactToWeapon))
		{
			FlagWeaponDrawn = ((Weapon.bEmitWeaponDrawn) && (weapon.MaxRange >= VSize(pawn.Location - Player.Location)));
			if ((FlagWeaponDrawn) && (Weapon.Concealability == CONC_ALL)) FlagWeaponDrawn = false;
			
			if ((pawn.FutzTimer <= 0) && (FlagWeaponDrawn) && ((pawn.AICanSee(Player, , false, true, false, false) > 0)))
			{
				PawnHookPawn = VMDBufferPawn(closeTarget);
				
				if (Pawn.bReactFutz)
				{
					if (PawnHookPawn == None) FlagGivesAShit = true;
					else if (!PawnHookPawn.bNoGunQuestioning) FlagGivesAShit = true;
				}
		...
		if ((weapon != None) && (!weapon.bHandToHand) && (!bUseOldTarget) && (bShowFocuser))
		{
			...
			if (weapon.MaxRange >= VSize(target.Location - Player.Location))
			{
				...
				mult = FMax(mult, corner+2.5); //MADDERS: Used to be 4.
		...
		if (ConvertVectorToCoordinates(target.Location, boxCX, boxCY))
		{
			...
			if ((bTargetActive) && (Player.Level.Netmode == NM_Standalone))
			{
				...
				else
				{
					...
					gc.DrawPattern(boxTLX, boxTLY, w, h, 0, 0, None); // Transcended - Transparent instead of solid
				...
				if (target.IsA('Pawn'))
					str = target.FamiliarName; // Transcended - Fixed name
				else if (target.IsA('DeusExCarcass'))
				{
					str = DeusExCarcass(target).FamiliarName;
					if (str == "")
						str = DeusExCarcass(target).itemName;
				}
				else if (target.IsA('DeusExDecoration'))
					str = DeusExDecoration(target).itemName;
				else if (target.IsA('DeusExProjectile'))
					str = DeusExProjectile(target).itemName;
				else if (target.IsA('Inventory')) // Transcended - Added
					str = Inventory(target).itemName;
				else if (target.IsA('Mover')) //MADDERS, 4/19/24: Stop printing mover names FFS.
					str = "Terrain";
				else
					str = target.GetItemName(String(target.Class));
				
				Str = VMDFilterInvalidNames(Str);
				
				// print disabled robot info
				if (target.IsA('Robot') && (Robot(target).EMPHitPoints == 0))
					str = str $ " (" $ msgDisabled $ ")";
				else if (target.IsA('Robot') && (Robot(target).RebootTimer > 0))
					str = str $ " (" $ msgRebooting $ ")";
				...
				//G-Flex: don't show health for carcasses or invincible decorations
				if (target.IsA('DeusExCarcass'))
				{
					bShowHealth = false;
				}
				else if (target.IsA('DeusExDecoration'))
				{
					if (DeusExDecoration(target).bInvincible)
						bShowHealth = false;
					else
						bShowHealth = true;
				}
				...
				if (bShowHealth)
				{
					...
					if (target.IsA('Pawn'))
					{
						...
						{
							EMPMult = Float(Robot(Target).EMPHitPoints) / float(Robot(Target).StartingEMPHitPoints);
							if ((EMPMult < 0.01) && (Robot(Target).EMPHitPoints > 0)) EMPMult = 0.01;
						}
						
						if (VMDBufferPawn(Target) != None)
						{
							mult = Float(VMDBufferPawn(Target).Health) / float(VMDBufferPawn(Target).StartingHealthValues[6]);
							if ((Mult < 0.01) && (VMDBufferPawn(Target).Health > 0)) Mult = 0.01;
							
							if (VMDBufferPawn(Target).EnergyMax > 0)
							{
								EnergyMult = VMDBufferPawn(Target).Energy / VMDBufferPawn(Target).EnergyMax;
								if ((EnergyMult < 0.01) && (VMDBufferPawn(Target).Energy > 0)) EnergyMult = 0.01;
							}
						}
						else
						{
							mult = Float(Pawn(target).Health) / Float(Pawn(target).Default.Health);
							if ((Mult < 0.01) && (Pawn(Target).Health > 0)) Mult = 0.01;
						}
					}
					else if (target.IsA('DeusExDecoration'))
					{
					...
					if (targetLevel == 0)
					{
						...
						if (Robot(Target) != None)
						{
							...
							Str = Str $ CR() $ EMPStr @ MsgEMPHealth;
						}
						
						if ((VMDBufferPawn(Target) != None) && (VMDBufferPawn(Target).EnergyMax > 0))
						{
							if (EnergyMult >= 0.66)
							{
								EnergyStr = msgHigh;
							...
							Str = Str $ CR() $ EnergyStr @ MsgEnergyPercent;
						}
					}
					else
					{
						...
						if (target.IsA('Pawn') && !target.IsA('Robot') && !target.IsA('Animal'))
						{
							if (VMDBufferPawn(Target) != None)
							{
								...
								str = str @ msgOverall;
								mult = Float(Pawn(target).HealthHead) / float(VMDBufferPawn(Target).StartingHealthValues[0]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgHead;
								mult = Float(Pawn(target).HealthTorso) / float(VMDBufferPawn(Target).StartingHealthValues[1]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgTorso;
								mult = Float(Pawn(target).HealthArmLeft) / float(VMDBufferPawn(Target).StartingHealthValues[2]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgLeftArm;
								mult = Float(Pawn(target).HealthArmRight) / float(VMDBufferPawn(Target).StartingHealthValues[3]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgRightArm;
								mult = Float(Pawn(target).HealthLegLeft) / float(VMDBufferPawn(Target).StartingHealthValues[4]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgLeftLeg;
								mult = Float(Pawn(target).HealthLegRight) / float(VMDBufferPawn(Target).StartingHealthValues[5]);
								str = str $ CR() $ Int(mult * 100.0) $ msgPercent @ msgRightLeg;
								mult = x;
								
								if (VMDBufferPawn(Target).EnergyMax > 0)
								{
									EnergyStr = Int(EnergyMult * 100.0) $ MsgPercent @ MsgEnergyPercent;
									Str = Str $ CR() $ EnergyStr;
									
									if ((VMDBufferPawn(Target).ActiveAugStr != "") && (TargetLevel > 1))
									{
										Str = Str $ CR() $ CR() $ VMDBufferPawn(Target).ActiveAugStr;
									}
								}
								
								for (TItem = VMDBufferPawn(Target).Inventory; TItem != None; TItem = TItem.Inventory)
								{
									if (ChargedPickup(TItem) != None)
									{
										if (!bPickupTagged)
										{
											Str = Str $ CR();
										...
										Str = Str $ CR() $ SprintF(MsgWearing, ChargedPickup(TItem).ItemName);
									}
								}
							}
							else
						...
						else
						{
							if (Robot(Target) != None)
							{
								EMPStr = Int(EMPMult * 100.0) $ MsgPercent @ MsgEMPHealth;
							}
				...
				if (targetLevel > 1)
				{
					...
					if (target.IsA('Pawn'))
					{
						...
						//MADDERS, 1/16/21: Don't do this, please.
						//target.GetItemName(String(Pawn(target).Weapon.Class));
			...
			else
			{
				...
				else if ((target.Level.NetMode != NM_Standalone) && (Human(target) != None))
				{
					// str = "";
					str = Human(target).PlayerReplicationInfo.PlayerName;
					str = str $ CR();
					if ((DeusExGameInfo(player.DXGame).bIsTeamGame()) && (DeusExGameInfo(player.DXGame).ArePlayersAllied(Human(target),player)))
					{
						// str = Human(target).HealthHead $ "/" $ Human(target).HealthTorso $ "/" $ Human(target).HealthLegLeft;
						str = str $ int(100 * (Human(target).Health / Float(Human(target).Default.Health))) $ "% HP";
						str = str $ " | ";
						str = str $ int(100 * (Human(target).Energy / (Human(target).Default.Energy))) $ "% BE";
						str = str $ CR();
					}
					str = str $ "Streak: ";
					str = str $ int(Human(target).PlayerReplicationInfo.Streak);
					gc.SetTextColor(crossColor);
					gc.GetTextExtent(0, w, h, str);
					x = boxCX - w/2;
					y = boxTLY - h - margin;
...
function DrawVisionAugmentation(GC gc)
{
	...
	if (visionLevel >= 1 || VMDUseAlternateVision())
	{
		// shift the entire screen to dark red (except for the middle box)
      		if (!VMDUseAlternateVision())
      		{
...
//MADDERS: No junk names, please.
function string VMDFilterInvalidNames(String S)
{
	switch(CAPS(S))
	{
		case "DEADBODY":
		case "SCRIPTEDPAWN":
		case "DEFAULT DECORATION NAME - REPORT THIS AS A BUG":
		case "DEFAULT FAMILIAR NAME - REPORT THIS AS A BUG":
		case "DEFAULT UNFAMILIAR NAME - REPORT THIS AS A BUG":
		case "DEFAULT PROJECTILE NAME - REPORT THIS AS A BUG":
			return "";
		break;
	}
...
defaultproperties
{
     ...
     colYellow=(R=57,G=167,B=142) //Fuck it. Make this Zomp now.
     colHiddenDoor=(R=229,G=192,B=229) //Faint purple.
}
