...
// ConPlayBase
//=============================================================================
class ConPlayBase extends Actor;
...
function TerminateConversation(optional bool bContinueSpeech, optional bool bNoPlayedFlag)
{
	...
	if ((!bContinueSpeech) && (Player != None))
...
function SetPlayedFlag()
{
	...
	if (con != None)
	{
		...
		//MADDERS, 10/29/21: For faster one-line tweaking if we change convo prefix later.
		TSearch = "FemJC";
		
		//MADDERS, 10/29/21: Mark male equivalent convos as done when female convos are done.
...
function EEventAction SetupEventTransferObject( ConEventTransferObject event, out String nextLabel )
{
	...
	*/
	...
	if ((event.fromActor != None) && (Class<Ammo>(event.giveObject) == None)) // Transcended - Don't do this for ammo, always spawn new to prevent the AI giving the player ALL of this ammo
	...
	if (invItemFrom == None)
	{
		...
		if (DeusExAmmo(InvItemFrom) != None)
		{
			DeusExAmmo(InvItemFrom).AmmoAmount = Clamp(DeusExAmmo(InvItemFrom).Default.AmmoAmount * Max(Event.TransferCount, 1), 0, DeusExAmmo(InvItemFrom).VMDConfigureMaxAmmo());
		}
		else if ((DeusExWeapon(InvItemFrom) != None) && (DeusExWeapon(InvItemFrom).AmmoName != None))
		{
			DeusExWeapon(InvItemFrom).PickupAmmoCount = Clamp(DeusExWeapon(InvItemFrom).PickupAmmoCount, 0, DeusExWeapon(InvItemFrom).AmmoName.Default.MaxAmmo);
			DeusExWeapon(invItemFrom).bItemRefusalOverride = true;
		}
		else if (DeusExPickup(invItemFrom) != None)
		{
            		DeusExPickup(invItemFrom).bItemRefusalOverride = true;
		}
		bSpawnedItem = True;
	}
	
	//MADDERS, 9/12/21: Reset this stupid bullshit as well.
	if (Weapon(InvItemFrom) != None)
	{
		Weapon(invItemFrom).PickUpAmmoCount = Weapon(InvItemFrom).Default.PickupAmmoCount;
	}
	...
	if ((invItemTo == None) && (DeusExPlayer(event.toActor) != None) && (FindAdvancedInventorySlot(DeusExPlayer(Event.ToActor), InvItemFrom, True) == False))
	...
	if (invItemTo != None)
	{
		...
		if ((DeusExPlayer(Event.FromActor) != None) && (DeusExPickup(InvItemFrom) == None || DeusExPickup(InvItemFrom).NumCopies <= event.TransferCount))
		{
		...
		{
			AmmoType = Ammo(DeusExPlayer(event.ToActor).FindInventoryType(Weapon(invItemTo).Default.AmmoName));
		...
		else
		{
			...
			if (itemsTransferred == 0)
			{
				if ((bSpawnedItem) && (InvItemFrom != None))
				{
					InvItemFrom.Destroy();
				}
				return nextAction;
	...
	else
	{
		...
		//MADDERS: Whewie. This is *very* reckless, Eidos. Just GiveTo? God damn.
		if (VMDBufferPlayer(Event.ToActor) != None)
		{
			VMDBufferPlayer(Event.ToActor).VMDUpdateInventoryJank();
		}
		
		if ((invItemFrom.IsA('DeusExPickup')) && (DeusExPickup(invItemFrom).bCanHaveMultipleCopies) && (DeusExPickup(invItemFrom).NumCopies > event.transferCount))
		{
			itemsTransferred = event.TransferCount;
			invItemTo = Spawn(event.giveObject);
			
            		if (DeusExPickup(invItemTo) != None)
			{
				DeusExPickup(invItemTo).bItemRefusalOverride = true;
			}
			
			invItemTo.GiveTo(Pawn(event.toActor));
			DeusExPickup(invItemFrom).NumCopies -= event.transferCount;
			DeusExPickup(InvItemFrom).UpdateBeltText();
		...
		//MADDERS: Try various rotations now, of course.
		if (DeusExPlayer(event.toActor) != None)
			FindAdvancedInventorySlot(DeusExPlayer(Event.ToActor), InvItemFrom, False);
	...
	{
		if (InvItemTo.PickupMessage != "")
		{
			// Need to do this seperately
			if ((Ammo(invItemTo) != None) && (Ammo(invItemTo).PickupViewMesh != Mesh'TestBox'))
			{
				if (DeusExAmmo(InvItemTo) != None)
				{
					if (DeusExAmmo(InvItemTo).bNameCaseSensitive)
					{
						Player.ClientMessage(InvItemTo.PickupMessage @ InvItemTo.ItemArticle @ InvItemTo.ItemName @ "("$ItemsTransferred$")", 'Pickup');
					}
					else
					{
						Player.ClientMessage(InvItemTo.PickupMessage @ InvItemTo.ItemArticle @ class'VMDStaticFunctions'.Static.VMDLower(InvItemTo.ItemName) @ "("$ItemsTransferred$")", 'Pickup');
					}
				}
				else
				{
			...
			else if ((Ammo(invItemTo) == None))
			{
				if (DeusExPickup(InvItemTo) != None)
				{
					if (DeusExPickup(InvItemTo).bNameCaseSensitive)
					{
						Player.ClientMessage(InvItemTo.PickupMessage @ InvItemTo.ItemArticle @ InvItemTo.ItemName, 'Pickup');
					}
					else
					{
						Player.ClientMessage(InvItemTo.PickupMessage @ InvItemTo.ItemArticle @ class'VMDStaticFunctions'.Static.VMDLower(InvItemTo.ItemName), 'Pickup');
					}
				}
				else if (DeusExWeapon(InvItemTo) != None)
				{
					if (DeusExWeapon(InvItemTo).bNameCaseSensitive)
					{
						Player.ClientMessage(InvItemTo.PickupMessage @ InvItemTo.ItemArticle @ InvItemTo.ItemName, 'Pickup');
					}
					else
...
*/
	...
	if (invItemTo.IsA('Weapon'))
	{
		...
		{
			AmmoType = Ammo(transferTo.FindInventoryType(Weapon(invItemTo).Default.AmmoName));
			
			if ( AmmoType != None )
			{
				//MADDERS, 9/12/21: Whoops. Turns out me fixing the pickup ammo count on these weapons is totally fucking shit up.
				//Ever been given 12 grenades instead of 3? Yeah, well, thanks mappers for setting Count=4.
				itemsTransferred = Weapon(invItemTo).Default.PickUpAmmoCount * (event.transferCount - 1);
				
				if (DeusExAmmo(AmmoType) != None)
				{
					//MADDERS, 2/4/21: Don't add impossible amounts of ammo. kthx.
					ItemsTransferred = Clamp(ItemsTransferred, 0, DeusExAmmo(AmmoType).VMDConfigureMaxAmmo());
				}
	...
	else if ((invItemTo.IsA('DeusExPickup')) && (DeusExPickup(invItemTo).bCanHaveMultipleCopies))
	{
		...
		if (bSpawned)
		{
			...
			if (event.transferCount > 1)
			{
				...
				if (DeusExPickup(invItemTo).NumCopies > DeusExPickup(invItemTo).VMDConfigureMaxCopies())
				{
					itemsTransferred = DeusExPickup(invItemTo).VMDConfigureMaxCopies();
					DeusExPickup(invItemTo).NumCopies = DeusExPickup(invItemTo).VMDConfigureMaxCopies();
				}
			}
		}
		...
		else
		{
			...
			if ((DeusExPickup(invItemTo).NumCopies + itemsTransferred) > DeusExPickup(invItemTo).VMDConfigureMaxCopies())
				itemsTransferred = DeusExPickup(invItemTo).VMDConfigureMaxCopies() - DeusExPickup(invItemTo).NumCopies;
	...
	else if ((invItemTo.IsA('DeusExPickup')) && (!bSpawned))
	{
		...
		if ((DeusExPlayer(transferTo) != None) && (FindAdvancedInventorySlot(DeusExPlayer(TransferTo), GiveItem, False)))
		{
			itemsTransferred = 1;
			giveItem.GiveTo(transferTo);
		}
...
function EEventAction SetupEventAddCredits( ConEventAddCredits event, out String nextLabel )
{
	...
	// Transcended - New
	if (!conWinThird.creditsWindow.IsVisible())
		conWinThird.creditsWindow.Show();
	conWinThird.creditsWindow.SetText(creditsText @ player.Credits);
...
function EEventAction SetupEventCheckPersona( ConEventCheckPersona event, out String nextLabel )
{
	...
	switch(event.personaType)
	{
		...
		case EP_SkillPoints:
			...
			{
				PersonaValue = Player.SkillPointsTotal - VMDBufferPlayer(Player).SkillPointsSpent;
			}
			else
			{
...
function TurnSpeakingActors(Actor speaker, Actor speakingTo)
{
	...
	// In 3+ people conversations the player might not be talking or talked to
	// Thus the player's ConversationActor might be old, making them face some
	// other random NPC.
	// Fix this by always facing the talking person if we're not talking or
	// being talked to.
	if (DeusExPlayer(speaker) == None && DeusExPlayer(speakingTo) == None)
	{
		for (i=0; i<ArrayCount(ConActorsBound); i++)
		{
			if (DeusExPlayer(ConActorsBound[i]) != None)
				DeusExPlayer(ConActorsBound[i]).ConversationActor = speaker;
		}
	}
}
...
function TurnActor(Actor turnActor, Actor turnTowards)
{
	...
	if (DeusExPlayer(turnActor) == None)
	{
		...
		if ((turnActor != None) && (turnActor.IsA('ScriptedPawn')) && (turnTowards != turnActor))
...
//MADDERS: For inventory swoocey shit.
