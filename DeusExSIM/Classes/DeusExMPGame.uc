...
//=============================================================================
class DeusExMPGame extends DeusExGameInfo;
...
function bool RestartPlayer( pawn aPlayer )	
{
    ...
    if ((DeusExRootWindow(PlayerToRestart.RootWindow) != None) && (DeusExRootWindow(PlayerToRestart.RootWindow).AugDisplay != None))
    {
	DeusExRootWindow(PlayerToRestart.RootWindow).AugDisplay.Show();
    }
...
function Killed( pawn Killer, pawn Other, name damageType )
{
   ...
   if ((Killer.bIsPlayer) && (Other.bIsPlayer))
   {
 	    ...
 	    log(Killer.PlayerReplicationInfo.PlayerName$"("$Killer.PlayerReplicationInfo.PlayerID$") killed "$Other.PlayerReplicationInfo.PlayerName@"("$Other.PlayerReplicationInfo.PlayerID$")" $ otherPlayer.killProfile.methodStr);
		for (CurPawn = Level.PawnList; CurPawn != None; CurPawn = CurPawn.NextPawn)
		{
			if ((CurPawn.IsA('DeusExPlayer')) && (DeusExPlayer(CurPawn).bAdmin))
				DeusExPlayer(CurPawn).LocalLog(Killer.PlayerReplicationInfo.PlayerName$"("$Killer.PlayerReplicationInfo.PlayerID$") killed "$Other.PlayerReplicationInfo.PlayerName@"("$Other.PlayerReplicationInfo.PlayerID$")" $ otherPlayer.killProfile.methodStr);
		}
