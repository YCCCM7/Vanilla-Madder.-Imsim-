//=============================================================================
// VMDMenuAugsSelector. Lightweight and to the point, I'd hope.
//=============================================================================
class VMDMenuAugsSelector expands MenuUIWindow;
...
function ActivateAugmentation(Augmentation TargetAug)
{
	...
	VMA = VMDBufferAugmentation(TargetAug);
	if ((TargetAug != None) && (Player != None) && (Player.AugmentationSystem != None))
	{
		if ((VMDBufferAugmentationManager(Player.AugmentationSystem) == None) && (VMA != None) && (VMA.bPassive))
		{
			VMA.bDisabled = !VMA.bDisabled;
			if (VMA.bDisabled)
			{
				Player.PlaySound(Sound'AugDeactivate', SLOT_None);
				Player.ClientMessage(SprintF(class'VMDBufferAugmentationManager'.Default.MsgAugDisabled, VMA.AugmentationName));
			}
			else
			{
				Player.PlaySound(Sound'AugActivate', SLOT_None);
				Player.ClientMessage(SprintF(class'VMDBufferAugmentationManager'.Default.MsgAugEnabled, VMA.AugmentationName));
			}
...
function CreateControls()
{
	...
	if ((Player != None) && (Player.AugmentationSystem != None))
	{
		...
		for (i=0; i<ArrayCount(AugButtons); i++)
		{
			AugButtons[i] = VMDAugSelectorButton(NewChild(class'VMDAugSelectorButton'));
		...
		AugPage = VMDAugSelectorPageButton(NewChild(class'VMDAugSelectorPageButton'));
		...
		ExitButton = VMDAugSelectorPageButton(NewChild(class'VMDAugSelectorPageButton'));
		ExitButton.SetPos(AugButtonPos[6].X, AugButtonPos[6].Y);
		ExitButton.SetFakeIcon(Texture'VMDCloseAugPageIcon');
	...
	switch(Key)
	{
		...
		case JumpKey[2]:
			if (SelectedAug == 6)
			{
				...
				{
					VMDBufferPlayer(Player).LastBrowsedAugPage = CurPage;
