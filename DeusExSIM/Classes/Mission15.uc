...
function FirstFrame()
{
	...
	if (localURL == "15_AREA51_BUNKER")
	{
		...
		if (!flags.GetBool('Ray_dead'))
		{
			...
			{
				if ((WaltonSimons(Pawn) != None) && (!flags.GetBool('VMD_WaltonBunkerGoodies')))
				{
					...
					{
						WaltonSimons(Pawn).VMDInitializeSubsystems();
					}
					if (WaltonSimons(Pawn).AugmentationSystem != None)
					{
						WaltonSimons(Pawn).AugmentationSystem.ActivateAllEco();
					}
				}
			}
...
function Timer()
{
	...
	else if (localURL == "15_AREA51_ENTRANCE")
	{
			...
			flags.GetBool('EverettAppears'))
		{
			...
			{
				Morgan.EnterWorld();
				Morgan.bLookingForEnemy = false;
				Morgan.bLookingForLoudNoise = false;
				Morgan.bLookingForAlarm = false;
				Morgan.bLookingForDistress = false;
				Morgan.bLookingForProjectiles = false;
				Morgan.bLookingForShot = false;
	...
	else if (localURL == "15_AREA51_FINAL")
	{
			...
			flags.GetBool('TongAppears'))
		{
			...
			{
				Tracer.EnterWorld();
				Tracer.bLookingForEnemy = false;
				Tracer.bLookingForLoudNoise = false;
				Tracer.bLookingForAlarm = false;
				Tracer.bLookingForDistress = false;
				Tracer.bLookingForProjectiles = false;
				Tracer.bLookingForShot = false;
			...
			flags.GetBool('PaulAppears'))
		{
			if (flags.GetBool('PaulDenton_Dead'))
			{
				...
				{
					pawn.EnterWorld();
					pawn.bLookingForEnemy = false;
					pawn.bLookingForLoudNoise = false;
					pawn.bLookingForAlarm = false;
					pawn.bLookingForDistress = false;
					pawn.bLookingForProjectiles = false;
					pawn.bLookingForShot = false;
					pawn.bLookingForInjury = false;
