...
// MenuScreenSaveGame
//=============================================================================
class MenuScreenSaveGame extends MenuScreenLoadGame;
	...
	switch(msgBoxMode)
	{
		case MB_Overwrite:
			if ( buttonNumber == 0 ) 
			{
				...
				{
					Player.dataLinkPlay.AbortAndSaveHistory();
					Player.SaveSkillPoints();
				}
				SaveGame(saveRowId);
				if (Player.DatalinkPlay != None)
				{
					Player.RestoreSkillPoints();
				}
			}
			else
			{
...
function UpdateSaveInfo(int rowId)
{
	...
	if (VMP != None)
	{
		if (VMP.bNGPlusTravel)
		{
			TNGPlus = VMDBufferPlayer(Player).SaveGameNGPlus;
		}
	}
	...
	if (rowId == newSaveRowID)
	{
		...
		winSaveInfo.SetText(Sprintf(LocationLabel, MissionLocation)$TNGPlus);
...
function NewSaveGame()
{
	...
	local string TNGPlus, TDiff, TGender, TClass, TChunker;
	local VMDBufferPlayer VMP;
	...
	if (VMP != None)
	{
		TClass = VMP.AssignedClass;
		if (TClass ~= "") TClass = "Custom";
		
		TDiff = VMP.AssignedDifficulty;
		if (TDiff ~= "") TDiff = VMP.StrDifficultyNames[1];
		
		if (VMP.bAssignedFemale) TGender = VMP.SaveGameGenders[1];
		else TGender = VMP.SaveGameGenders[0];
	...
	editName.SetSelectedArea(0, Len(TChunker));
...
function ConfirmSaveGame()
{
	...
	if ((freeDiskSpace < minFreeDiskSpace) && (freeDiskSpace >= 0))
	{
...
function PerformSave()
{
	...
	if ((VMP != None) && (class'VMDStaticFunctions'.Static.UseGallowsSaveGate(VMP)))
	{
...
function EnableButtons()
{
	...
	if ((VMP != None) && (class'VMDStaticFunctions'.Static.UseGallowsSaveGate(GetPlayerPawn())))
	{
