...
//=============================================================================
class ActorDisplayWindow extends Window;
...
var bool		 bShowNeatName;
var bool 		 bShowCustom;
var string 		 customProp;
...
function Bool IsDataVisible()
{
	return bShowData;
}

function ShowCustomProp(bool bShow)
{
	bShowCustom = bShow;
}
...
function ShowNeatNames(bool bShow)
{
	bShowNeatName = bShow;
}

function Bool IsNeatNameVisible()
{
	return bShowNeatName;
}
...
function DrawWindow(GC gc)
{
	...
	foreach player.AllActors(viewClass, trackActor)
	{
		...
		if (bPointValid)
		{
			...
			if (bShowCustom || bShowData)
			{
				str = str $ "|cffffff"$customProp$"=";
				str = str $ trackActor.GetPropertyText(customProp) $ CR();
			}
			...
			gc.SetFont(Font'TechSmall');
			if (bShowNeatName)
				gc.DrawText(leftX-40, topY-140, 80+rightX-leftX, 135, GetPlayerPawn().GetItemName(trackActor.FamiliarName));
			else
