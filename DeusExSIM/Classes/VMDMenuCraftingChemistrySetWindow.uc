...
function UpdateInfo()
{
	...
	local int QuanReqA, QuanReqB, QuanReqC, ItemOwnedA, ItemOwnedB, ItemOwnedC, TarItemOwned,
			ChemicalsCost, ChemicalsGain, ModifiedChemicalsGain, QuanMult, CraftQuan, BreakdownQuan, ComplexityNeeded, SkillNeeded, SkillLevel;
	...
	else if ((VMP != None) && (VMP.SkillSystem != None))
	{
		if ((TarTile != None) && (TarTile.CurItem != None))
		{
			...
			if (bHasBreakdown)
			{
				...
				ChemicalsGain = CF.GetMedicalBreakdownPrice(None, TarItem) * BreakdownGainTweak;
				ModifiedChemicalsGain = CF.GetMedicalBreakdownPrice(VMP, TarItem) * BreakdownGainTweak;
				
				BreakdownQuan = CF.GetMedicalBreakdownQuanNeeded(TarItem);
				
				RequirementLabels[5].SetText(SprintF(MsgBreakdownGain, ModifiedChemicalsGain*QuanMult));
			...
			if (ComplexityNeeded > 1+(int(bHasMedbot)*2))
			{
			...
			if (bHasBreakdown)
			{
				if (QuanMult > 1)
				{
					BuildStr[2] = SprintF(MsgBreakdownParamsA, BreakdownQuan, ModifiedChemicalsGain)$SprintF(MsgBreakdownParamsB, QuanMult, BreakdownQuan*QuanMult, ModifiedChemicalsGain*QuanMult);
				}
				else
				{
	...
	if ((bCanCraft) && (CF != None) && (SelectedTile != None) && (SelectedTile.CurItem != None) && (VMP.SkillSystem != None))
	{
		...
		if (VMP.bUseInstantCrafting || bHasMedbot)
		{
			...
			if (TItem != None)
			{
				...
				VMP.PlaySound(Sound'MedicalCraftingMini', SLOT_None,,, 1024, RandomPitch());
				RefreshItemTiles();
			}
		}
		else
...
{
	local bool bHasTalent;
	local int SkillLevel, SkillNeeded, QuanMult, ChemicalsGain, ModifiedChemicalsGain, BreakdownQuan, AmountNotAdded;
	...
	if ((bCanBreakdown) && (CF != None) && (SelectedTile != None) && (SelectedTile.CurItem != None) && (VMP.SkillSystem != None))
	{
		...
		if (VMP.bUseInstantCrafting || bHasMedbot)
		{
			...
			VMP.LastMedicalBreakdown = string(TarItem);
			
			AmountNotAdded = (ModifiedChemicalsGain*QuanMult) - (VMP.MaxChemicals - VMP.CurChemicals);
			if (AmountNotAdded > 0)
			{
				if (ChemCrammer != None)
				{
			...
			VMP.PlaySound(Sound'MedicalCraftingMini', SLOT_None,,, 1024, RandomPitch());
			VMP.AddChemicals(ModifiedChemicalsGain*QuanMult, false);
			RefreshItemTiles();
		}
		else
		{
			VMP.VMDCraftingStartBreakdown(TarItem, BreakdownQuan, ModifiedChemicalsGain, true, false, QuanMult);
			AddTimer(0.1, False,, 'DoPop');
		}
	}
}
