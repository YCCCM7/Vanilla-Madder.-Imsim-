...
var TextWindow creditsWindow;					// Credits displayed at the upper left during choices
...
event InitWindow()
{
	...
	// creditsWindow.SetBackground(conTexture);
	creditsWindow.SetBackground(None);
	creditsWindow.SetFont(Font'DeusExUI.FontConversation');
	creditsWindow.SetTextMargins(20,20);
	...
	// To not display it when the convo opens.
	creditsWindow.Hide();
...
function CalculateWindowSizes()
{
	...
	if (bForcePlay)
	{
		...
		//MADDERS: Strange, but this is our subtitles fix.
...
// Transcended - Copy of above for virtual key presses
function bool KeyboardButtonActivated(int choice)
{
	local int buttonIndex;
	...
	for ( buttonIndex=0; buttonIndex<numChoices; buttonIndex++ )
	{
		if (conChoices[buttonIndex] == conChoices[choice])
		{
			// Turn the cursor back off
			root.ShowCursor(False);
			conPlay.PlayChoice( ConChoice(conChoices[buttonIndex].GetUserObject()) );
...
//			break;
			...
			case IK_Down:
				bHandled = False;
			break;
			// Transcended - Let numbers select a choice too
			case IK_1:
			case IK_NUMPAD1:	
				if (numChoices > 0)
					KeyboardButtonActivated(0);
				else
					conPlay.PlayNextEvent();
			break;
			case IK_2:
...
function AbortCinematicConvo()
{
	...
	//MADDERS, 4/23/24: Please stop causing weird shit. We never asked for your help.
	if ((VMDBufferPlayer(Player) != None) && (VMDBufferPlayer(Player).VMDHasCinematicClickObjection()))
	{
		VMDBufferPlayer(Player).Fire();
		return;
	}
