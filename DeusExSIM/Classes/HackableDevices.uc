...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	local int DrawMultitool;
	local float dotp, HackTweak2, TSkill;
	local string msg;
	local Vector X, Y, Z;
	local AutoTurret TTurret;
	local AutoTurretGun TGun;
	...
	if (!bDone)
	{
		...
		else if ((TTurret != None) && (!TTurret.bTrackPlayersOnly) && (VMP != None) && (VMP.HasSkillAugment('TagTeamMiniTurret')))
		{
		...
		else if ((VMP != None) && (VMP.InHand == None) && (VMP.InHandPending == None))
		{
			DrawMultitool = VMP.VMDShouldDrawMultitool(Self);
			if (DrawMultitool > 0)
			{
				if (DrawMultitool == 2)
				{
					VMP.PutInHand(VMP.FindInventoryType(class'Multitool'));
				}
				Msg = "";
			}
		}
