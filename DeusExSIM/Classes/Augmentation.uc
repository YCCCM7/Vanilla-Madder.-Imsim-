...
{
Begin:
	log("** AUGMENTATION: .Active should never be called!"@Class.Name);
}
...
function Activate()
{
	...
	local VMDBufferPawn TVMBP;
	
	// can't do anything if we don't have it
	if (!bHasIt)
		return;
	
	VMA = VMDBufferAugmentation(Self);
	if (VMA != None) TVMBP = VMA.VMBP;
	
	//MADDERS: Don't allow spam attempts to work, or other stupid BS.
	if ((Player != None) && ((Player.Energy <= 0 && AugLight(Self) == None) || Player.IsInState('Paralyzed') || Player.IsInState('Dying') || IsInState('TrulyParalyzed'))) return;
	
	//MADDERS, 12/27/23: Pawn support.
	if ((TVMBP != None) && (TVMBP.Energy <= 0 || TVMBP.IsInState('Dying'))) return;
	...
	if (IsInState('Inactive'))
	{
		...
		if (Player != None)
		{
			...
			{
				Player.PlaySound(ActivateSound, SLOT_None,,,, 1.3 * GSpeed);
			}
			else
			{
		...
		else if (TVMBP != None)
		{
			...
			{
				TVMBP.SoundVolume = 160;
				TVMBP.AmbientSound = LoopSound;
			}
		}
		...
		if (TVMBP != None)
		{
			//MADDERS, 12/28/23: Update our printed augs, to save performance where possible.
			TVMBP.UpdateActiveAugsStr();
		}
	}
}
...
function Deactivate()
{
	...
	if (IsInState('Active'))
	{
		...
		if (Player != None)
		{
			...
			if (VMDMechAugmentation(Self) != None)
			{
				Player.PlaySound(DeactivateSound, SLOT_None,,,, 1.3 * GSpeed);
			}
			else
			{
				Player.PlaySound(DeactivateSound, SLOT_None,,,, GSpeed);
			}
		}
		else if (TVMBP != None)
		{
			if ((TVMBP.AugmentationSystem != None) && (TVMBP.AugmentationSystem.NumAugsActive() == 0))
			{
				TVMBP.SoundVolume = 64;
				TVMBP.AmbientSound = None;
			}
...
simulated function bool CanBeUpgraded()
{
	...
	// the maximum level
	if ((CurrentLevel < MaxLevel) && (Player != None))
	{
...
simulated function bool UpdateInfo(Object winObject)
{
	...
	if (VMA != None)
	{
		if (bUsingMedbot)
		{
			...
			winInfo.AppendText(winInfo.CR() $ winInfo.CR() $ VMA.VMDGetAdvancedDescription());
		}
		else
		{
			winInfo.SetText(VMA.VMDGetAdvancedDescription());
		}
	}
	else
	{
