...
var localized string ShowHealthInfoLabel, ShowStatInfoLabel,
			StrStatusTitle,
			StrOverdoseDesc[4],
			StrPoisonDesc[2], StrFireDesc[2], StrNanoVirusDesc[2], StrDeathNegateDesc[2],
			StrKillswitchDesc[9],
			StrMayhemDesc[3],
			strMedkitReductionDesc[4],
			StrHungerDesc[3],
			StrBloodSmellDesc[4], StrFoodSmellDesc[6], StrZymeSmellDesc[2], StrSmokeSmellDesc[4],
			StrAddictionDesc[4], StrWithdrawalDesc[5], StrSubstanceNames[6],
			StrCombatStimDesc[3], StrMedigelDesc[2], StrPharmacistDesc[2], StrSodaBuffDesc[2], StrCandyBuffDesc[2], StrCigarettesBuffDesc[2], StrZymeBuffDesc[2],
...
function VMDShowStatusInfo()
{
	...
	{
		bUnderwaterMedkitBad = true;
	}
	if ((VMP.VMDDoAdvancedLimbDamage()) && (VMP.HealthArmLeft <= 0) && (VMP.HealthArmRight <= 0))
	{
		bArmsBroken = true;
	}
	
	if (bUnderwaterMedkitBad || bArmsBroken)
	{
		TStr[0] = StrMedkitReductionDesc[0];
		if (!bArmsBroken)
		{
			TStr[1] = SprintF(StrMedkitReductionDesc[1], VMP.CalculateSkillHealAmount(30));
		}
		else if (!bUnderwaterMedkitBad)
		{
			TStr[1] = SprintF(StrMedkitReductionDesc[2], VMP.CalculateSkillHealAmount(30));
		}
		else
		{
	...
	if (VMP.bHungerEnabled)
	{
		...
		HungerTimeLeft = (VMP.HungerCap - VMP.HungerTimer) / Sqrt(VMP.TimerDifficulty) / 60 / 0.65;
	...
	if (VMP.bSmellsEnabled)
	{
		...
		if (VMP.ZymeSmellLevel >= 45)
		{
			SmellUnits = int(VMP.ZymeSmellLevel);
			SmellUnitsThresh[0] = 45;
		...
		if (VMP.SmokeSmellLevel >= 45*SmellMults[0])
		{
			SmellUnits = int(VMP.SmokeSmellLevel);
			SmellUnitsThresh[0] = int(45*SmellMults[0]);
			SmellUnitsThresh[1] = int(90*SmellMults[0]);
			
			if (VMP.SmokeSmellLevel >= SmellUnitsThresh[1])
			{
				TStr[0] = StrSmokeSmellDesc[1];
				TStr[1] = SprintF(StrSmokeSmellDesc[3], SmellUnits, SmellUnitsThresh[1]);
			}
			else
			{
				TStr[0] = StrSmokeSmellDesc[0];
	...
	if (Auras[0] != None)
	{
		...
		{
			TStr[1] = SprintF(StrCombatStimDesc[1], int((float(Auras[0].Charge) / 40.0) + 0.5), THeal);
		}
	...
	{
		TStr[0] = StrDodgeRollCooldownDesc[0];
