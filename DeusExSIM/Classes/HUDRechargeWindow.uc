...
var localized String ReadyLabel, ReadyLabelUnlimited;
...
var PersonaActionButtonWindow BtnCrafting;
...
event Tick(float deltaSeconds)
{
	...
	if (repairBot == None || ((repairBot != None) && (RepairBot.Health <= 0 || RepairBot.EMPHitPoints <= 0 || RepairBot.bDeleteMe)))
	{
		repairBot = None;
		root.PopWindow();
		bTickEnabled = False;
	}
	else if (RepairBot.RechargeLeft > 0)
	{
		if (lastRefresh >= refreshInterval)
		{
			lastRefresh = 0.0;
			UpdateBioWindows(); //MADDERS, 1/3/21: Update this in real time. Why not, right?
...
function UpdateBioWindows()
{
	...
	if (Player != None)
	{
		if (Player.EnergyMax == 100)
			...
			winBioBarText.SetText(String(Int(player.Energy)) $ "/" $ String(Int(player.EnergyMax)) $ " ~ " $ "(" $ String(Int(energyPercent)) $ "%" $ ")");
	}
...
function UpdateRepairBotWindows()
{
	...
	if (repairBot != None)
	{
		...
		{		
			...
			if (RepairBot.bHubBot)
			{
				winRepairBotBarText.SetText(ReadyLabelUnlimited);
			}
			else
			{
				winRepairBotBarText.SetText(SprintF(ReadyLabel, RepairBot.RechargeLeft));
			}
		}
		else
		{
...
function CreateButtons()
{
	...
	winActionButtons.SetSize(241, 16);
...
function VMDInvokeCraftingScreen()
{
	if (VMP != None)
	{
		VMP.VMDInvokeToolboxWindow(true, RepairBot);
	}
}
...
defaultproperties
{
     ...
     texBackground(0)=Texture'VMDHUDRepairBotBackground_1'
