...
function CalcTrace(float deltaTime)
{
	...
	StartTrace = Location;
	EndTrace = Location + 50000 * vector(Rotation);
	HitActor = None;
	...
	for (i=0; i<ArrayCount(spot); i++)
	{
		...
		//MADDERS: Don't let null laser locations go unchecked.
		if (HitLocation == vect(0,0,0)) HitLocation = EndTrace;
...
function Tick(float deltaTime)
{
	...
	if (bIsOn)
	{
		...
		else if (proxy != None)
		{
			...
			if (proxy.DistanceFromPlayer < 1920)
				bFrozen = False;
			else
			{
