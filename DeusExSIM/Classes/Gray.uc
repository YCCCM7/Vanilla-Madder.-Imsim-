...
function Tick(float deltaTime)
{
	...
	if ((damageTime >= damageInterval) && (!IsInState('Dying')))
	{
		damageTime = 0;
		foreach VisibleActors(class'DeusExPlayer', player, damageRadius)
			if ((player != None) && (FastTrace(Player.Location, Location)))
...
defaultproperties
{
     ...
     //MADDERS: Grays are incredibly foreign to our world. They aren't tuned for our types of smells.
     SmellTypes(0)=
     SmellTypes(1)=
     SmellTypes(2)=
     SmellTypes(3)=
     ...
     Alliance=Gray
     DrawType=DT_Mesh
     Mesh=LodMesh'TranscendedModels.TransGray'
     AmbientGlow=12
     SoundRadius=14
     SoundVolume=255
