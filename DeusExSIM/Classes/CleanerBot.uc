...
function Tick(float deltaSeconds)
{
	...
	local FleshFragment TFrag;
	local pawn fearPawn;
	
	Super.Tick(deltaSeconds);
	
	//MADDERS: Don't run from stuff if we're braindead.
	if (EMPHitPoints <= 0 || RebootTimer > 0) return;
	...
	if (blotchTimer > 0.3)
	{
		...
		foreach RadiusActors(Class'FleshFragment', TFrag, CollisionRadius*2)
		{
			if (!TFRag.bDeleteMe)
			{
				PlaySound(TFrag.ImpactSound, SLOT_None);
				PlaySound(Sound'Spark1', SLOT_Misc);
				TFrag.Destroy();
				TakeDamage(2, Self, Location, vect(0,0,0), 'EMP');
				if (EMPHitPoints > 0)
				{
					RebootTimer = 3.0;
				}
...
{
	if ((EMPHitPoints > 0) && (FleePawn != Self))
	{
...
defaultproperties
{
     ...
     DrawType=DT_Mesh
     Mesh=LodMesh'TranscendedModels.TransCleanerBot'
     SoundRadius=16
     SoundVolume=128
