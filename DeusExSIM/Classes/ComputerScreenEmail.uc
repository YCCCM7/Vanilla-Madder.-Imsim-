...
var MenuUIActionButtonWindow     btnAddLog;
...
var localized String AddToLog;

var int lastFocusRowId;
...
function SetNetworkTerminal(NetworkTerminal newTerm)
{
	...
	if (VMP != None)
	{
		if ((VMP.bEmailSavingOption) && (!WinTerm.bHacked || VMP.HasSkillAugment('ComputerScaling')))
		{
			btnAddLog = winButtonBar.AddButton(AddToLog, HALIGN_Left);
		}
	}
...
function SetCompOwner(ElectronicDevices newCompOwner)
{
	...
	else
	{
		...
		if (btnAddLog != None)
			btnAddLog.SetSensitivity(False);
	}
}
...
function bool ButtonActivated( Window buttonPressed )
{
	...
	switch( buttonPressed )
	{
		...
		case btnAddLog:
			if (winEmail.GetText() != "" && winEmail.GetText() != NoEmailTodayText)
			{
				emailInfoIndex = Int(lstEmail.GetField(lastFocusRowId, 2));
				
				note = player.GetNote(emailInfo[emailInfoIndex].emailName);

				if (note == None)
					note = player.AddNote(winEmail.GetText(),, True, "Emails");
				Note.SetTextTag(emailInfo[emailInfoIndex].emailName);
			}
			btnAddLog.SetSensitivity(False);
			break;
...
event bool ListSelectionChanged(window list, int numSelections, int focusRowId)
{
	...
	if (btnAddLog != None)
	{
		if (emailInfo[emailInfoIndex].emailName == '05_Email08')	// Hack
			btnAddLog.SetSensitivity(False);
		else
			btnAddLog.SetSensitivity(True);
	}
