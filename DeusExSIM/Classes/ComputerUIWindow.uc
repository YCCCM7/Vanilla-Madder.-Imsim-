...
//VMD shit. So far only hacks?
var string VMDQueuedAction, VMDHCGetSel;
...
{
	//MADDERS, 4/1/24: Delay this action to fix a bug with hotel carone cameras getting too ambitious, thank you.
	VMDQueuedAction = Action;
	if ((WinTerm != None) && (WinTerm.CompOwner != None))
	{
		VMDHCGetSel = WinTerm.CompOwner.GetPropertyText("HCSel");
	}
	AddTimer(0.008, false, 0, 'VMDExecuteScreenclose');
...
function SetCompOwner(ElectronicDevices newCompOwner)
{
	...
	if (IsA('HCComputerScreenSecurity'))
	{
		VMDHCGetSel = WinTerm.CompOwner.GetPropertyText("HCSel");
		AddTimer(0.008, false, 0, 'VMDHackUpdateCameras');
		AddTimer(0.008, false, 0, 'InitCameras');
	}
}
...
function ProcessDeusExText(Name textName, optional TextWindow winText)
{
	...
	OName = TextName;
	//MADDERS, 11/04/21: Convert usage to female text flag when female.
...
//MADDERS, 4/1/24: Turns out this not being delayed crashes us in some scenarios, such as HotelCarone. This is us post-delay.
function VMDExecuteScreenClose()
{
	local string HackVal;
	
	if (winTerm != None)
	{
		winTerm.CloseScreen(VMDQueuedAction);
		VMDQueuedAction = "";
	}
}

//MADDERS, 4/1/24: Intergalactic trickshot. Fix this side-effect of fixing the HC crash.
//I hate it, I hate it, boy do I hate it... But fuck it, if you're reading this, it works.
function VMDHackUpdateCameras()
{
	local string CamStr, TStr, GetIndex, GetCam;
	local SecurityCamera TCam;
	local Window TWindow;
	
	if ((IsA('HCComputerScreenSecurity')) && (WinTerm != None) && (WinTerm.CompOwner != None))
	{
		CamStr = GetPropertyText("HCCamera");
		GetIndex = VMDHCGetSel;
		forEach AllObjects(class'Window', TWindow)
		{
			TStr = "HCComputerSecurityCameraWindow'"$string(TWindow)$"'";
			if (TStr == CamStr)
			{
				TWindow.SetPropertyText("ViewIndex", GetIndex);
				TWindow.AddTimer(0.008, false, 0, 'UpdateCameraStatus');
			}
		}
	}
}
