...
//=============================================================================
class ComputerScreenATMWithdraw extends ComputerUIWindow;
...
function SetCompOwner(ElectronicDevices newCompOwner)
{
	...
	//MADDERS: Like our cousin VM, this is no longer scaled by skill.
	//However, the relevant skill augment changes that.
	BalanceModifier = 1.0;
	
	if (WinTerm.bHacked)
	{
		BalanceModifier = FMax(0.25, winTerm.GetSkillLevel()) * 0.25;
		if ((VMP != None) && (VMP.HasSkillAugment('ComputerATMQuality')))
		{
			BalanceModifier = FMax(0.5, winTerm.GetSkillLevel()) * 0.5;
		}
	}
