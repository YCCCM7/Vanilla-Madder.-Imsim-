//=============================================================================
// MobileProjectileGenerator.
//=============================================================================
class MobileProjectileGenerator extends Effects;
...
{
	local int i, j, count;
	local float speed, timeVal, GSpeed;
	local vector dir;
	local actor spawnee;
...
*/
	...
	if (time >= CheckTime)
	{
		...
		{
			if (spawnSound != None)
				PlaySound(spawnSound, SLOT_Misc,,, SpawnSoundRadius, GSpeed);
			...
			for (i=0; i<count; i++)  // Number of spews for this tick
			{
				for (j=0; j<numPerSpawn; j++)  // Number of spawns per spew
				{
					...
					if (spawnee != None)
					{
						...
						//MADDERS: The whole reason we exist, thank you very much.
						if (Base != None)
						{
							spawnee.Velocity += Base.Velocity;
						}
						//MADDERS, 1/10/21: Also do this now, for owned halon gas.
						if (Owner != None)
						{
							Spawnee.SetOwner(Owner);
						}
