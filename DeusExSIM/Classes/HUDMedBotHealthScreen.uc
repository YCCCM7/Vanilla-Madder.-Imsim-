...
//MADDERS, 1/12/21: Proper UI labels FFS.
var localized string SecondsNeverLabel, MedbotDepleted;
...
{
	UpdateRegionWindows(); //MADDERS, 1/3/21: Update this in real time. Why not, right?
	
	// Transcended - Added, fixes CTD
	// Maybe check for bDeleteMe?
	if (MedBot == None || ((MedBot != None) && (MedBot.Health <= 0 || MedBot.EMPHitPoints <= 0 || MedBot.bDeleteMe)))
	{
		medBot = None;
		root.PopWindow();
		bTickEnabled=False;
	}
	else if (Medbot.HealingLeft > 0)
	{
		UpdateMedBotDisplay();
	}
}
...
function UpdateMedBotDisplay()
{
	...
	if (medBot != None)
	{
		...
		if (MedBot.HealingLeft <= 0)
		{
			winHealthBar.SetCurrentValue(0);
			winHealthBarText.SetText(SecondsNeverLabel);
			InfoText = InfoText $ MedbotDepleted;
		...
		else
		{
			...
			{		
				...
				if (Medbot.bHubBot)
				{
					winHealthBarText.SetText(ReadyLabelUnlimited);
				}
				else
				{
					winHealthBarText.SetText(SprintF(ReadyLabel, MedBot.HealingLeft));
				}
...
function MedBotHealPlayer()
{
	...
	//MADDERS, 6/5/22: Update info accordingly.
	if (bShowingStatuses)
	{
		VMDShowStatusInfo();
	}
