...
event bool VirtualKeyPressed(EInputKey key, bool bRepeat)
{
	...
	{	
		...
		case IK_Escape:
			if (MenuMain(Self) != None)
			{
			 GetPlayerPawn().Slomo(1.0);
			}
...
{
	local class<MenuUIWindow> LoadClass;
	
	// Check to see if the player has already ran the training mission
	// or been prompted
	//--------------------
	//MADDERS, 12/23/22: We're going to assume that you've played DX before playing VMD now.
	//People keep expecting this tutorial to teach them about VMD specifically, and that's not true.
	...
	{*/
		//MADDERS: Character customization, ahoy!
		if (!Player.IsA('Trestkon'))
		{
			player.ConsoleCommand("Open 00_CharacterSetup");
		}
		else
		{
	...
	switch(messageBoxMode)
	{
		...
		case MB_AskToTrain:
			if (buttonNumber == 0)
				...
				root.InvokeMenuScreen(Class'VMDMenuSelectCustomDifficulty');
				//root.InvokeMenuScreen(Class'MenuSelectDifficulty');
			break;
