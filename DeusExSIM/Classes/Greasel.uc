...
{
	if ((damageType == 'TearGas') || (damageType == 'HalonGas') || (damageType == 'PoisonGas') || (damageType == 'DrugDamage'))
		return false;
	else
...
function TweenToAttack(float tweentime)
{
	...
	{
		if (HasAnim('WaterAttack'))
		{
			TweenAnimPivot('Tread', tweentime, GetSwimPivot());
			//TweenAnimPivot('WaterAttack', tweentime, GetSwimPivot());
		}
		else
		{
			TweenAnimPivot('Tread', tweentime, GetSwimPivot());
...
function PlayAttack()
{
	if ((Region.Zone.bWaterZone) && (HasAnim('WaterAttack')))
	{
		PlayAnimPivot('Attack');
		//PlayAnimPivot('WaterAttack',,, GetSwimPivot()); // Fixed anim
	}
	else
	{
		PlayAnimPivot('Attack');
...
defaultproperties
{
     ...
     //MADDERS: Faster and invincible greasels. Fuck logic.
     bInvincible=False
     ...
     InitialAlliances(1)=(AllianceName=Player,AllianceLevel=-1.000000,bPermanent=True)
     ...
     Alliance=Greasel
     DrawType=DT_Mesh
     Mesh=LodMesh'TranscendedModels.TransGreasel'
