...
//MADDERS additions!
var localized string MsgLastAlarmTime, MsgLastAlarmHackTime, MsgLockoutTime, MsgLastHackTime;
...
function BeginAlarm()
{
	...
	{
		LastAlarmTime = AlarmTimeout * 0.5;
	}
	else
	{
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if (player != None)
	{
		if (bLockedOut)
		{
			...
			Elapsed = LockoutTime;
			if (elapsed > 0)
				player.ClientMessage(Sprintf(msgLockedOut, Int(elapsed)));
			else
				bLockedOut = False;
		}
...
function string VMDGetItemName()
{
	...
	if (Abs(LastAlarmTime - LastHackTime) < 1.0)
	{
		if ((LastAlarmTime > 0) && (AmbientSound != None))
		{
			Ret = Ret$Chr(13)$Chr(10)$SprintF(MsgLastAlarmHackTime, int(LastAlarmTime + 0.5));
		}
	}
	else
	{
		if (LastHackTime > 0)
		{
			Ret = Ret$Chr(13)$Chr(10)$SprintF(MsgLastHackTime, int(LastHackTime + 0.5));
		}
		if ((LastAlarmTime > 0) && (AmbientSound != None))
		{
			Ret = Ret$Chr(13)$Chr(10)$SprintF(MsgLastAlarmTime, int(LastAlarmTime + 0.5));
		}
	}
	if (LockoutTime > 0)
	{
		Ret = Ret$Chr(13)$Chr(10)$SprintF(MsgLockoutTime, int(LockoutTime + 0.5));
	}
