...
simulated function float GetSkillLevelValue(class SkillClass)
{
	...
	if (aSkill != None)
	{
		...
		//MADDERS, 7/24/23: Keypads talent now doubles effectiveness of electronic bypassing.
		if (VMDBufferPlayer(Player) != None)
		{
			if (SkillTech(ASkill) != None)
			{
				if (VMDBufferPlayer(Player).HasSkillAugment('ElectronicsHackingPotency2'))
				{
					RetVal *= 2;
				}
				else if (VMDBufferPlayer(Player).HasSkillAugment('ElectronicsHackingPotency1'))
				{
					RetVal *= 1.5;
				}
			}
			else if (SkillMedicine(ASkill) != None)
			{
				if (VMDBufferPlayer(Player).HasSkillAugment('MedicineStress'))
				{
					RetVal += 0.17;
				}
				if (VMDBufferPlayer(Player).HasSkillAugment('MedicineWraparound'))
				{
					RetVal += 0.5;
				}
			}
		...
		else
		{
			if (SkillTech(ASkill) != None)
			{
				if (class'VMDSkillAugmentManager'.Static.StaticSkillAugmentAssumed('ElectronicsHackingPotency2'))
				{
					RetVal *= 2;
				}
				else if (class'VMDSkillAugmentManager'.Static.StaticSkillAugmentAssumed('ElectronicsHackingPotency1'))
				{
					RetVal *= 1.5;
				}
			}
			...
			{
				if (class'VMDSkillAugmentManager'.Static.StaticSkillAugmentAssumed('MedicineStress'))
				{
					RetVal += 0.17;
				}
				if (class'VMDSkillAugmentManager'.Static.StaticSkillAugmentAssumed('MedicineWraparound'))
				{
					RetVal += 0.5;
				}
			}
