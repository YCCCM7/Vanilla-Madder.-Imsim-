...
function FirstFrame()
{
	...
	if (localURL == "11_PARIS_CATHEDRAL")
	{
		...
		{
			if ((GuntherHermann(Pawn) != None) && (!flags.GetBool('VMD_GuntherCathedralGoodies')))
			{
				GuntherHermann(Pawn).AddToInitialInventory(class'BioelectricCell', 10);
				flags.SetBool('VMD_GuntherCathedralGoodies', True,, 12);
			}
		}
	}
...
function Timer()
{
	...
	else if (localURL == "11_PARIS_CATHEDRAL")
	{
			...
			!flags.GetBool('MS_M11WaltAppeared'))
		{
			...
			foreach AllActors(class'MJ12Troop', MJ12, 'TROOPSPAM')
			{
				MJ12.EnterWorld();
				MJ12.SetPhysics(PHYS_Falling);
			}
