...
function UpdateCameraStatus()
{
	...
	//MADDERS, 12/2/23: No longer static-ify the camera. We're toggling AI in our headcanon.
	else if (!camera.bActive)
	{
	...
	else
	{
		...
		winCamera.SetDefaultTexture(None);
		winCamera.Lower();
		//btnCamera.winStatic.Lower();
		//btnCamera.winStatic.Hide();
		SetTitle(CameraLabel @ "|&" $ String(viewIndex + 1) @ ":" @ ComputerSecurity(compOwner).Views[viewIndex].titleString);
