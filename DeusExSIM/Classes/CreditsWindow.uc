...
event DestroyWindow()
{
	...
	if (bLoadIntro)
	{
		if ((VMDBufferPlayer(Player) != None) && (DeusExRootWindow(Player.RootWindow) != None))
		{
			//VMDBufferPlayer(Player).StartNewGamePlus();
			DeusExRootWindow(Player.RootWindow).AddTimer(0.1, False,, 'RemoteStartNewGamePlus');
		}
		else
		{
