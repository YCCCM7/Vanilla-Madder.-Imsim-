...
function VMDSetupCameraIndices()
{
	//Vanilla Matters
	local int i, j;
    	...
    	for ( i = 0; i < 4; i++ )
	{
        	for( j = 0; j < 3; j++ )
		{
            		choiceWindows[i].Cameras[j] = winCameras[j];
        	}
    	}
}
...
function CreateChoices()
{
	...
	for(choiceIndex=0; choiceIndex<arrayCount(choices); choiceIndex++)
	{
		if (choices[choiceIndex] != None)
		{
			...
			if (ComputerSecurityChoice_Turret(NewChoice) != None)
			{
				ComputerSecurityChoice_Turret(NewChoice).WinTerm = WinTerm;
			}
...
function SetNetworkTerminal(NetworkTerminal newTerm)
{
	...
	// Turrets are only usable at Advanced or higher 
   	...
   	if (Player.Level.NetMode == NM_Standalone)
   	{
		...
		//MADDERS, 1/24/21: This is bullshit. Change this in real time, as this apparently never sets properly.
		if (ComputerSecurityChoice_Turret(ChoiceWindows[3]) != None)
		{
			ComputerSecurityChoice_Turret(ChoiceWindows[3]).WinTerm = NewTerm;
		}
		
		//MADDERS, 1/24/21: Proper toggle for this with skill augments disabled.
      		if ((bTurretOK) && (winTerm.bHacked))
		{
...
event bool VirtualKeyPressed(EInputKey key, bool bRepeat)
{
	...
	switch(key)
	{
		//MADDERS, 12/2/23: Restoring this button functionality. Annotated stuff is what DXT does, for whatever reason.
		/*case IK_Left:		
		case IK_Right:		
		case IK_Up:			
		case IK_Down:		
...
function ResetCameraRotations()
{
	...
	{
		if (winCameras[cameraIndex].camera != None)
			winCameras[cameraIndex].camera.bOverwriteControl = False;
    }
}
